...
\echapter{Surface Turbulent Flux Schemes}
%\addcontentsline{toc}{chapter}{地表湍流交换过程}

%\begin{地表湍流交换过程}
\section{基本理论}\label{基本理论}
\esection{Fundamental Theory}
\begin{mymdframed}{代码}
...
事实上，$L$的初始猜测可由总体理查德森数$R_{\mathrm{ib}}$与$\zeta_{\mathrm{a}}$的关系得到\citep{arya2001introduction}。$R_{\mathrm{ib}}$表达为
...
\esection{New Theory with Large Eddy Effects}
\begin{mymdframed}{代码}
...
\esection{Turbulent Flux for Bare Surfaces}
\begin{mymdframed}{代码}
...
陆地表面可分为被积雪覆盖与未被积雪覆盖两部分，其被积雪覆盖的地表面积比例$f_{\mathrm{sno}}$的计算参考章节~\ref{积雪覆盖比例}。对于无植被覆盖部分的陆地表面，湍流通量只存在于地表与大气之间，此部分动量通量$\tau$、感热通量$H$和水汽通量$E$表达为：
...
\esection{1-D Vegetation Turbulent Model}
\begin{mymdframed}{代码}
...
为了表达上的便利，我们引入对于植被表面蒸散发是否发生的判别因子$\delta$如下：
\begin{equation}
  \label{delta_cal}
  \delta=
  \begin{cases}
...
\esection{3-D Vegetation Turbulent Model}
\begin{mymdframed}{代码}
...
\esubsection{Roughness Scheme with Stability Consideration}
\begin{mymdframed}{代码}
...
\esubsection{One-Layer Canopy Turbulence Exchange}
...
\esubsection{Multi-Layer Canopy Turbulence Exchange}
{
  \begin{figure}[htbp]
    \centering
