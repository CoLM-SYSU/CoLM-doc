...
\begin{equation}
\begin{aligned}
 \frac{\partial L_{v}}{\partial T_{v}} &= -8 f_{sig} \left(1-\mu_{t h}\right)\left(\sigma T_{v}^{3}\right) \\
 \frac{\partial L_{v, s u n}}{\partial T_{v, s h a}} &= 0
\end{aligned}
\end{equation}
感热通量表达式$H_{pv}$已在章节~\ref{一维植被湍流交换模型} 给出，其对温度的变化率为：
\begin{equation}
\frac{\partial H_{p v}}{\partial T_{v}}=f_{sig} \rho_{atm} C_{pa} c_{v}^{h}
\end{equation}
对于水汽通量$E_{pv}$，虽然其表达式也已在章节~\ref{一维植被湍流交换模型} 给出，但这里为了表达上的便利，
...
\begin{equation}
\begin{aligned}
E_{pvt} &= -\frac{f_{sig} \rho_{atm}}{c_{a}^{w}+c_{g}^{w}+c_{v}^{w}} \frac{\delta LAI\left(1-f_{w e t}\right)}{r_{b}+r_{s}}\left[c_{a}^{w} q_{atm}+c_{g}^{w} q_{g}-\left(c_{a}^{w}+c_{g}^{w}+c_{v}^{w}\right) q_{s a t}^{T_{v}}\right] \\ 
\frac{\partial E_{pvt}}{\partial T_{v}} &= \frac{f_{sig} \rho_{atm} \delta LAI\left(1-f_{w e t}\right)}{r_{b}+r_{s}} \frac{c_{a}^{w}+c_{g}^{w}}{c_{a}^{w}+c_{g}^{w}+c_{v}^{w}} \frac{d q_{s a t}^{T_{v}}}{d T_{v}}
\end{aligned}
\end{equation}
当$ E_{pvt} \geq f_{sig} E_{pvt,max}$ 即叶片最大蒸腾率(\unit{kg.m^{-2}.s^{-1}})时，
\begin{equation}
\begin{aligned}
E_{pvt} &= f_{sig} E_{pvt, \max } \\ 
\frac{\partial E_{pvt}}{\partial T_{v}} &= 0
\end{aligned}
\end{equation}
对于叶片的蒸发通量：
\begin{equation}
\begin{aligned}
E_{pva} &= -\frac{f_{\text {sig }} \rho_{atm}}{c_{a}^{w}+c_{g}^{w}+c_{v}^{w}} \frac{\left[1-\delta\left(1-f_{w e t}\right)\right](LAI+SAI)}{r_{b}}\left[c_{a}^{w} q_{atm}+c_{g}^{w} q_{g}\right. \\ 
&\mathrel{\phantom{=}}\left.-\left(c_{a}^{w}+c_{g}^{w}+c_{v}^{w}\right) q_{s a t}^{T_{v}}\right] \\ 
\frac{\partial E_{pva}}{\partial T_{v}} &= \frac{f_{\text {sig }} \rho_{atm}\left[1-\delta\left(1-f_{w e t}\right)\right](LAI+SAI)}{r_{b}} \frac{c_{a}^{w}+c_{g}^{w}}{c_{a}^{w}+c_{g}^{w}+c_{v}^{w}} \frac{d q_{s a t}^{T_{v}}}{d T_{v}}
\end{aligned}
\end{equation}
当$E_{pva} \geq \frac{W_{can}}{\Delta t} \frac{LAI+SAI}{LAI+SAI}$即叶片最大蒸发率(\unit{kg.m^{-2}.s^{-1}})时，
\begin{equation}
  \begin{aligned}
   E_{pva} &= \frac{W_{can}}{\Delta t} \\ 
   \frac{\partial E_{pva}}{\partial T_{v}} &= 0
  \end{aligned}
\end{equation}
于是对于叶片的总水汽通量：
\begin{equation}
  \begin{aligned}
   E_{pv} &= E_{pvt}+E_{pva} \\ 
   \frac{\partial E_{pv}}{\partial T_{v}} &= \frac{\partial E_{pvt}}{\partial T_{v}}+\frac{\partial E_{pva}}{\partial T_{v}}
  \end{aligned}
\end{equation}
