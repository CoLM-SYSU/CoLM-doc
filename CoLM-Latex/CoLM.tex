% !TEX encoding = UTF-8 Unicode
\documentclass[a4paper,12pt,twoside]{report}
\usepackage[left=2.5cm,right=2.5cm,top=3cm,bottom=3cm]{geometry}
\usepackage[dvipsnames]{xcolor}
\usepackage[UTF8,heading=true,scheme=chinese]{ctex}
\ctexset{
    chapter/number = \arabic{chapter},
    chapter/numberformat = \color{blue}\zihao{1},
    appendix={
    %   name={附录},
       %numbering=false,
       number={\Alph{chapter}}
    }
}

\usepackage[nottoc]{tocbibind} % used to add bibliography to toc with correct bookmark and page number
   
%\usepackage[T1]{fontenc}
\newcommand{\arial}[1]{{\fontencoding{T1}\usefont{T1}{phv}{m}{n}{#1}}}
\newcommand{\courier}[1]{{\fontencoding{T1}\usefont{T1}{pcr}{m}{n}{#1}}}
\newcommand{\Times}[1]{{\fontencoding{T1}\usefont{T1}{ptm}{m}{n}{#1}}}
\newcommand{\Rom}[1]{{\fontencoding{T1}\usefont{T1}{cmr}{m}{n}{#1}}}
% \usepackage{wasysym}

\input{CoLM.preamble}
\setlength{\parskip}{0em}
\setcounter{secnumdepth}{3}
\usepackage{enumitem}
\usepackage{wasysym}
\usepackage{textcomp}
\usepackage{rotating}
\usepackage{amssymb}
\usepackage{afterpage} % avoid blank page before landscape environment
\usepackage{pdflscape}
\usepackage{xr}
\usepackage{color}
\usepackage{textcomp}
\usepackage{threeparttable}
\usepackage{array}
\usepackage{longtable} % for 'longtable' environment
\usepackage{threeparttablex} % for 'ThreePartTable' environment
\usepackage{makecell}
\raggedbottom

\usepackage{siunitx}
%\PassOptionsToPackage{hyphens}{url}
\usepackage[%hidelinks
   citecolor=Blue,linkcolor=Blue,urlcolor=Blue,colorlinks=True,breaklinks=true]{hyperref}
\usepackage{xurl}
\usepackage{setspace}
\usepackage{doi}
\setstretch{1.5}
\usepackage{bookmark}

\begin{document}
%\include{uhead.sty}

\title{\huge {\bf 通用陆面模式 2024}\\[3em]
%\vspace{2mm}
\fontsize {22}{24}
\bf{ Description of The Common Land Model (CoLM2024)}\\[3in]
\fontsize {20}{23}%\\[3in]
% \vskip 3in
}

\author{%
 \large{ 中山大学 }\\[0.1in]
 {\bf 大气科学学院}\\[1in]
% \vskip 1in
 \upshape
 \large%\\[0.5in]
% \vskip 0.5in
2024 年 08 月%
}

%\normallinespacing
\maketitle
\preface
%\input{acknowledgements/acknowledgements}
%\input{symbols/symbols}
%\input{dedication/dedication}
%\input{quotes/quotes}

\listoftodos[待核查/修改列表]
\body
\part{引言}
\include{引言/引言}

\part{模式构架与基础数据}
\include{模式构架与基础数据/模式构架}
\include{模式构架与基础数据/基础数据}

\part{地表通量方案}\label{part:flux}
\include{地表通量方案/辐射通量方案}
\include{地表通量方案/地表湍流通量方案}
\include{地表通量方案/气孔导度和光合作用}
\include{地表通量方案/植被水力模式}
\include{地表通量方案/降水与地表的能量交换}

\part{植被冠层、积雪和土壤温度计算方案}\label{part:temp}
\include{植被冠层、积雪和土壤温度计算方案/植被叶片温度计算}
\include{植被冠层、积雪和土壤温度计算方案/雪盖土壤热力过程}

\part{植被冠层、积雪和土壤水分计算方案}\label{part:SPC}
\include{植被冠层、积雪和土壤水分计算方案/植被冠层水分}
\include{植被冠层、积雪和土壤水分计算方案/积雪土壤水分}
\include{植被冠层、积雪和土壤水分计算方案/积雪的水文过程}

\part{水文过程}\label{part:hydro}
\include{水文过程/耦合径流模型的产流和汇流过程}
% \include{水文过程/径流模式CaMa-Flood}
\include{水文过程/基于物理过程的侧向流模拟}
\include{水文过程/湖泊模式}
\include{水文过程/冰川模式}
\include{水文过程/湿地模式}

\part{生物地球化学循环过程}\label{part:BGC}
\include{生物地球化学循环过程/碳氮库结构}
\include{生物地球化学循环过程/植被生物地球化学循环过程}
\include{生物地球化学循环过程/土壤凋落物生物地球化学循环过程}
\include{生物地球化学循环过程/火灾}
\include{生物地球化学循环过程/生物地球化学循环预热加速}

\part{人类活动}\label{part:human}
\include{人类活动/城市模式}
\include{人类活动/作物模式}
\include{人类活动/水库模式}
\include{人类活动/土地利用与土地覆盖变化模拟}

%\part{尺度转换}\label{part:scaling}
%\include{尺度转换/土壤属性参数尺度转换}
%\include{尺度转换/植被属性参数尺度转换}
%\include{尺度转换/大气强迫降尺度}

\clearpage
\phantomsection % Correct the link in toc for Appendix
\addcontentsline{toc}{part}{附录}
\part*{附录}

%\cleardoublepage
\appendix
\clearpage
%\addcontentsline{toc}{chapter}{附录}
\include{附录/附录}

\bookmarksetup{startatroot}
\addtocontents{toc}{\bigskip}

\clearpage
\renewcommand{\bibname}{参考文献}
%\addcontentsline{toc}{chapter}{参考文献}
\bibliographystyle{agufull08-mod}
\bibliography{References/References}

\clearpage
\input{致谢/致谢}
\addcontentsline{toc}{chapter}{致谢}

\end{document}
