...
假设到达地表的净水流通量为$G_{\mathrm{water}}$。 一个模式计算单元内，饱和区域的地表水全部转化为径流流走，非饱和区域的地表水，部分入渗到土壤中，剩余部分转化为径流，总的地表径流$r_{\rm surface}$为：
\begin{equation}
...
\end{equation}
假设到达地表的净水流通量为$G_{\mathrm{water}}$，则该方案的地表产流$r_{\mathrm{surface}}$考虑三种情况：
\begin{equation}
r_{\mathrm{surface}} = 
\begin{cases}
G_{\mathrm{water}} & \text{当 } z_{\mathrm{wt,max}} \leq 0 \\
G_{\mathrm{water}} - w_{{satint}} + w_{{int}} & \text{当 } z_{\mathrm{wt,init}} + G_{\mathrm{water}} > z_{\mathrm{wt,max}} \\
G_{\mathrm{water}}- w_{{satint}} + w_{{int}} + w_{{satint}} \times {{I}_{{tmp}}}^{1 + B_{{VIC}}} & \text{其他情况}
\end{cases}
\end{equation}
其中：
\begin{equation}
{I}_{tmp} = 1 - \frac{\left( z_{\mathrm{wt,init}} + {G}_{\mathrm{water}} \right)}{z_{\mathrm{wt,max}}}
\end{equation}
