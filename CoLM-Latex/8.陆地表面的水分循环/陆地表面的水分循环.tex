...
\caption{1 穿透系数$\delta_P$与叶面积指数的关系及其变动范围。}
...
CoLM假设网格内的降水的分布是不均匀的。其中对流降水面积的网格占比$I_c$可以由下式给出
\begin{equation}
...
\end{equation}
其中，$θ_(s,i)$ 为第$ i$ 层土壤的饱和体积含水量，$ q_{(i-1\/2)}^{(n+1)}$为第$ i $层土壤上边界处的水流通量，$q_(wf,i)^{(n+1)}$为饱和区域下边界处的水流通量。
...
汇流计算是通过耦合Dai Yamazaki等人于2011年提出的大尺度分布式汇流模型CaMa-Flood (Catchment-based Macro-scale Floodplain) 实现的\citep{yamazaki2011physically}。
...
CaMa-Flood模型的主要优势之一在于除了能够精确描述河道径流以外，还能够模拟包括漫滩水位和漫滩面积变化等洪泛过程。
...
CaMa-Flood模型的另一个优势在于它具有极高的模拟计算效率。通过引入次网格地形参数，复杂的漫滩淹没物理过程被合理地简化。
与此同时，通过实现局部惯性方程和自适应时间步长等物理方案\citep{bates2010}，
河流流量和蓄水量的预测计算成本得到压缩,有利于进行集成/长期实验等计算要求高的实验或者与陆面模式之间的动态耦合。
以下对CaMa-Flood内部各个模块进行详细的介绍。目前耦合版本陆面过程模式分系统已经包含CaMa-Flood v4.07版本。


\subsection{自适应时间步长的估算}
为避免固定时间步长计算所产生的数值振荡，提高数值方案稳定性，
CaMa-Flood 采用了\citet{bates2010}提出的基于局部惯性方程并满足Courant-Friedrichs-Lewy ($CFL$) 
...
在CaMa-Flood中指定网格的漫滩 (洪泛) 状态是通过计算该网格的蓄水总量得出的。如图\ref{fig:CaMa-Flood流域单位示意图}
...
CaMa-Flood假设不存在漫滩 (洪泛) 事件，上述参数则通过如下方程计算得出：
\begin{equation}
...
\subsection{河道径流流量计算}
CaMa-Flood分别计算了各流域单元向其下游单元的河流径流量和漫滩流量。
二者的计算均通过忽略如下St. Venant动量方程式第二项，得到径流计算所使用的局部惯性方程 \cite{bates2010}:
\begin{equation}
...
CaMa-Flood引入限流器的概念：当总出水量大于网格的总库存量时，CaMa-Flood使用修正系数对径流流量进行修正。
...
$Q_k^t$代表在时间t该单元从上游网格接收的河流径流流入流量 (河道内+漫滩)，$A_{ci}$ 是单元i的面积，$R_i^t$ 代表流域单元$i$的产流量。

\subsection{河网地图}
CaMa-Flood模拟所需的相关次网格地形参数是由90m超高精细分辨率的全球流动方向地图 (MERIT Hydro)\citep{yamazaki2019merit}，
水文调整高程$DEM$\citep{yamazaki2017high,yamazaki2012analysis}，以及河宽数据\citep{yamazaki2014development}; 
使用 \citet{yamazaki2009deriving} 开发的升尺度模式Flexible Location of Waterways (FLOW) 计算得出。
相关数据以纯“二进制”格式 ($nx*ny$)存储于在map/目录下。默认的数据存储顺序是从180W到180E，从90N到90S；数据的字节顺序是 “little endian”。
...
所有的地形相关参数均可由MERIT-hydro水文地形数据中获取。以下以全球0.25度的河网为例进行详细介绍。
{
\begin{figure}[]
\centering
...
在目录$glb_15min$下包含了全球0.25度分辨率的网格-矢量-混合(grid-vector-hybrid)河网图，
...
\label{tab:河网图及地形参数文件列表}
    \begin{tabular}[h]{p{2.5cm}p{1.5cm}p{2.5cm}p{4cm}p{2cm}p{1cm}}  %{@{}cccccc@{}}
    \toprule
    File              & Variable & Symbol                        & Description                                  & Unit    & Format  \\ \midrule
    params.txt        & -        & -                             & Map parameters                               & -       & text    \\
    nextxy.bin        & nextx    & jx                            & DownstreamX (rec=1)                         & -       & integer \\
    nexty             & jy       & Downstream Y (rec=2)          & -                                            & integer &         \\
    downxy.bin        & downx    & dx                            & Relative DownstreamX (rec=1)                & -       & integer \\
    downy             & dy       & Relative Downstream Y (rec=2) & -                                            & integer &         \\
    ctmare.bin        & ctmare   & Ac                            & Unit-catchment Area                          & m2      & real    \\
    elevtn.bin        & elevtn   & Z                             & Base Elevation                               & m       & real    \\
    rivlen.bin        & rivlen   & L                             & Channel Length                               & m       & real    \\
    rivhgt.bin        & rivhgt   & B                             & Channel Depth                                & m       & real    \\
    rivwth.bin        & rivwth   & W                             & Channel Width                                & m       & real    \\
    rivwth\_gwdlr.bin & rivwth   & W                             & Combined With (recommended)                  & m       & real    \\
    nxtdst.bin        & nxtdst   & X                             & Downstream Distance                          & m       & real    \\
    fldhgt.bin        & fldhgt   & Df                            & Floodplain Elevation profile (rec=1$\sim$10) & m       & real    \\
    rivman.bin        & rivman   & -                             & Manning’s Roughness                          & -       & real    \\
    bifori.txt        & -        & -                             & Bifurcation Channel Original Data            & -       & text    \\
    bifprm.txt        & -        & -                             & Bifurcation channel parameters               & -       & text    \\ \bottomrule
    \end{tabular}
    \end{table}
...
\label{tab:河网图及地形参数文件列表2}
    ...
    \toprule
    File             & Variable & Symbol                             & Description                                          & Unit     & Format  \\ \midrule
    grdare.bin       & grdare   & -                                  & Rectangular grid area (optional)                     & m2       & real    \\
    nxtdst\_grid.bin &          &                                    &                                                      &          &         \\
    rivlen\_grid.bin & nxtdst   & X                                  & Downstream Distance (grid center)                    & m        & real    \\
    rivlen           & L        & Channel Length (grid center)       & m                                                    & real     &         \\
    inpmat.bin       & inpx     & -                                  & Corresponding input grid X (rec=1)                   & -        & integer \\
    inpy             & -        & Corresponding input grid Y (rec=2) & -                                                    & integer  &         \\
    ctmare.bin       & inpa     & Aij                                & Area of input grid XY (rec=3)                        & m2       & real    \\
    diminfo.txt      & -        & -                                  & Dimension infomation                                 & text     &         \\
    lsmask.bin       & -        & -                                  & Land ID of corresponding hi resolution area Basin ID & -        & integer \\
    basin.bin        & -        & -                                  & Basin ID                                             & -        & integer \\
    bsncol.bin       & -        & -                                  & Basin Color Pattern for Visualization                & -        & integer \\
    lonlat.bin       & lon      & -                                  & Longitude, catchment outlet (rec=1)                  & deg      & real    \\
    lat              & -        & Latitude, catchment outlet (rec=2) & deg                                                  & real     &         \\
    uparea.bin       & uparea   & -                                  & Upstream Drainage Area                               & {[}m2{]} & real    \\ \bottomrule
    \end{tabular}
    \end{table}
...
任意流域单元(图\ref{fig:河网图及地形参数文件列表2})的漫滩面积可以由漫滩水深$D_f$ (m) 
以及累积分布函数 (CDF) 的经验函数生成 (图\ref{tab:河网图及地形参数文件列表2})。
每个网格的CDF函数的第10百分位数的10个值 (图\ref{tab:河网图及地形参数文件列表2}，红色圆点)分别存储在$fldhgt.bin$的十个参数之中
 (见表\ref{tab:河网图及地形参数文件列表}和 表\ref{tab:河网图及地形参数文件列表2})，文件格式与上述的$nextxy.bin$相同。
例如，$fldhgt.bin$的第三个参数表示流域单元30\%的面积被淹没时的该流域单元的平均洪水深度 (m)。
{
\begin{figure}[]
\centering
...
如图\ref{fig:CaMa-Flood流域单位示意图}所示，由于CaMa-Flood是基于流域单元进行计算的，而陆面模式是基于规则的经纬度网格，
...
其中$F_i$为进入流域单元i在单位时间 [$\rm m^3s^{-1}$]的输入水量，$A_i$, $j $是陆面模式的经纬度网格单元$j$在属于指定流域单元的面积大小,
 $N$是指属于指定流域单元陆面模式的经纬度网格的数量。该映射信息被存储在$test_***.bin$($***$是网格分辨率
 ，例如 $test_1deg.bin$)。同时，还需准备一个文本文件 (比如说$diminfo_test-1deg.txt$) 用来指定模拟的维度
  (比如说区域，分辨率，流域单元数量，输入经纬度网格数量，输入的映射文件名$test_***.bin$)。



在map目录中，CaMa-Flood还准备了一些用于生成映射矩阵和漫滩高程剖面曲线所需的高分辨率数据，如 (3sec，1min，15min，3min等目录之下)。
高分辨率数据被划分为$\$(TILE).\$(VAR).bin$，每个TILE的区域记录在“location.txt”文件之中；
...
陆地 (未调整) =1, 陆地 (调整并在CaMa中使用) =2, 通常网格=3，流域边界=5, 河道=10, 内陆流域出口=20, 与海洋连接的河口=25。具体文件列表见表\ref{tab:河网各数据文件}。
...
\begin{table}[]
    ...
    \caption{河网各数据文件}
    \label{tab:河网各数据文件}
    \begin{tabular}[h]{p{3.5cm}p{1.0cm}p{2.5cm}p{4cm}p{2cm}p{1cm}} %{@{}cccccc@{}} %
    \toprule
    File                & Variable & Symbol                        & Description                   & Unit      & Format \\ \midrule
...
大坝水库的基本信息来自全球大坝水库数据集GRanD\citet{lehner2011high}，GRanD version1.3包含全球7320座大坝及其相关水库的数据。
...
在0.25$^{cicle}$河网中共计识别到2169座水库，在0.05$^{cicle}$河网中共计识别到5715座水库(图\ref{fig:水库分布})。
...
防洪区：
\begin{equation}
\left\{\begin{array}{ll}Q=Q_{f}+k \frac{V-V_{f}}{V_{e}-V_{f}}\left(I-Q_{f}\right), & I \geq Q_{f} \\
     Q=Q_{n} \times 0.5+\left(\frac{V-V_{c}}{V_{e}-V_{c}}\right)^{2}\left(Q_{f}-Q_{n}\right), & I<Q_{f}\end{array}\right.
\end{equation}
兴利区：
\begin{equation}
\left\{\begin{array}{ll}Q=Q_{n} \times 0.5+\left(\frac{V-V_{c}}{V_{f}-V_{c}}\right)\left(Q_{f}-Q_{n}\right), & I \geq Q_{f} \\
     Q=Q_{n} \times 0.5+\left(\frac{V-V_{c}}{V_{e}-V_{c}}\right)^{2}\left(Q_{f}-Q_{n}\right), & I<Q_{f}\end{array}\right.
\end{equation}
控制区：
\begin{equation}
...
防洪/兴利区：
\begin{equation}
\left\{\begin{array}{c}Q=(1-R) I+R \times \frac{V-V_{c}}{V_{f}-V_{c}} \times\left(Q_{n}+D-\bar{D}\right), 
    \bar{D} / Q_{n}<1-M \\ Q=(1-R) I+R \times \frac{V-V_{c}}{V_{f}-V_{c}} \times Q_{n}\left(M+\frac{(1-M) D}{\bar{D}}\right),
     \bar{D} / Q_{n}>M\end{array}\right.
\end{equation}
控制区：
\begin{equation}
Q=Q_{n} \times\left(\frac{V}{V_{f}}\right)
