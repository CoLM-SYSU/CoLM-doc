...
\section{土壤表面反照率}
\begin{mymdframed}{代码}
反照率的计算公式(\ref{eq:soil_albedo1})--(\ref{alpha_sno_nir_dif})对应的代码文件为\texttt{MOD\_Albedo.F90}。
\end{mymdframed}
...
\end{equation}
其中下标$soi$表示土壤，$vis$和$nir$分别表示可见光和近红外波段，$dir$表示直射光，$\alpha_{sat}$和$\alpha_{d r y}$是根据土壤颜色分类，通过查找表得到的饱和($sat$)和干($dry$)土壤的反照率数值，$\theta_{l}$为表层土壤体积含水量。目前土壤颜色数据采用CLM4.5[TODO@yuan: 1) 更新为CLM5.0; 2) 添加引文。]全球20种颜色分类数据，包括可见光波段和近红外波段。漫射光与直射光相应波段一致。
