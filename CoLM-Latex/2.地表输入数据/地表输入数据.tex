...
土壤颜色（反照率）数据来自NCAR CLM5.0模式(mksrf\_soilcolor\_histclm52deg025\_earths\\tatmirca\_2005.c210808 版)，
...
该数据目前只发布了51.6$^\circ$N$\sim$51.6$^\circ$S范围内的数据，北半球高纬度地区数据缺失，因此目前该数据集仅作为备用数据。
\subsection{城市LAI/SAI数据}\label{城市LAISAI数据}
由于城市中建筑物的遮掩，卫星反演数据中城市$LAI$存在一定问题，目前模式中城市$LAI$数据利用以下公式根据0.5$^\circ$范围内的其它PFT (只考虑树，即1-9类PFT)的$LAI$/$SAI$，通过插值并加权平均的办法为城市地区$LAI$/$SAI$赋值：
\begin{equation}
