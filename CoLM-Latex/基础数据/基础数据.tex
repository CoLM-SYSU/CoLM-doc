...
闪电频率数据提供了3小时气候态的全球数据(单位：$flashes km^{-2} hr^{-1}$)，空间分辨率是1.875x1.915度，是对1995-2011年NASA LIS/OTD 2小时2.5度格点产品(版本v2.2)进行线性插值而得到(http://ghrc.msfc.nasa.gov)。 


\subsection{国民生产总值数据}\label{国民生产总值数据}
国民生产总值数据来源于IPCC-SRES的基准年份GDP数据\citep{van2007downscaling}，根据国家层面的世界银行世界发展指数计算得出\citep{WolrdBank2004,UNSTAT2005}。
