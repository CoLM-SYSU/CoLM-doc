...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{Figures/城市模式/CoLM城市模式概念图.png}
\caption{CoLM城市模式概念图。}
...
建筑物随机分布，包括建筑物位置及朝向。城市几何参数包括建筑物覆盖度$f_b$(即屋顶覆盖面积占比)，地面(透水、不透水面)覆盖度$f_g$($f_{gimp}$，$f_{gper}$)，建筑物高度$H$，建筑物高度与建筑物平均边长比$H/L$(建筑物底面考虑为正方形，边长为记为$L$)，植被冠层中心高度$H_v$，城市树木叶面积指数 LAI 及茎面积指数 SAI，植被(树)覆盖面积比例记为$f_v$。
...
现有的城市模式往往不对平均单栋建筑物进行表征，而是采用街谷宽度比$H/W$描述城市结构主要特征($W$表示楼与楼之间平均距离。在已知$f_b$和$H/W$参数，而未知$L$时，$L$可以简单计算为：
...
$S$表示阴影，$H/W$在推导过程中表示为$\rm HW$，$H/L$表示为$\rm HL$，总的直射入射和漫射入射太阳辐射能量通量采用单位能量1表示。

{
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\textwidth]{Figures/城市模式/CoLM城市模式建筑植被结构示意图.png}
...
即太阳辐射初始到达墙面和地面射辐射通量(未经散射)。$\mathbf{X}$为求解变量组成的向量($M_{wsun}$,$M_{wsha}$,$M_{gimp}$,$M_{gper}$)，
计算为：
...
在植被树冠中心高度$h_v$处，墙面的阴影比例可根据公式(\ref{S_w})计算，利用$\frac{H-h_v}{L}$代替$\rm HL$，记为$S_w^\prime$，即
\begin{equation}
S_{w}^{\prime}=1-\exp \left(-\frac{4}{\pi} \cdot \frac{f_{b}}{f_{g}} \frac{H-h_{v}}{L} \cdot \tan \left(\theta_{s}\right)\right)
\end{equation}
...
其中$\alpha$表示反照率(对于植被表示所有向外散射辐射比例)，不分区阳面和阴面墙，不区分漫射和直射照射，且反射后的辐射假设为漫射辐射。假设经过墙面、地面和植被之间多次散射，达到辐射平衡时的墙面和地面辐射出射通量为$M$，利用各组分之间的可视因子，可建立如下辐射平衡方程：
\begin{landscape}
\begin{equation}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{Figures/城市模式/CoLM城市模式无植被阻抗交换网络.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{Figures/城市模式/CoLM城市模式包含植被湍流交换阻抗网络.png}
...
\subsection{透水面温度计算}
\begin{mymdframed}{代码}
本节及下节对应的代码文件为\texttt{MOD\_Urban\_PerviousTemperature.F90}。
\end{mymdframed}

城市透水面即为土壤，与自然界土壤的热传递计算一致 (分层方案与土壤一致，土壤热力参数也从全球数据中读取)，只是上表面接收的短波、长波辐射及湍流交换通量(感热、潜热)由城市相应模块求解得到。

\subsection{不透水面温度计算}
\begin{mymdframed}{代码}
本节及下节对应的代码文件为\texttt{MOD\_Urban\_ImperviousTemperature.F90}。
\end{mymdframed}
...
\begin{mymdframed}{代码}
本节及下节对应的代码文件为\texttt{MOD\_Urban\_WallTemperature.F90}。
\end{mymdframed}

墙面(包括阴面墙、阳面墙)厚度从外部数据读取，同土壤一样，也分为10层，每层厚度一样，其热力参数从外部数据读取。
不同于透水面/不透水面，墙面不考虑积水、积雪覆盖，因此其热力属性完全由自身材料决定，同时也不考虑水传递、相变过程和潜热交换。

另一个不同之处在于最里(下)层的热量交换设置，对于土壤、不透水面，考虑最下一层无热量交换，但对于墙壁，考虑室内墙壁表面温度与最里层墙壁的热量交换。除此之外，其他方面及其求解过程与土壤温度类似。


\subsection{屋顶温度计算}

\begin{mymdframed}{代码}
本节及下节对应的代码文件为\texttt{MOD\_Urban\_RoofTemperature.F90}。
\end{mymdframed}
...
\end{equation}
其中$P$为格点内人口密度(\unit{pop.km^{-2}})，目前使用的是 LandScan 数据，
分辨率为1km，$H_M$为不同时刻人体排放的热量。人体排放的热量遵循一个基本假设：
...
\subsection{交通热}
交通热排放($Q_v$, \unit{W.m^{-2}})同样根据利用能源清单法进行计算，
采用Top-Down的方法，利用人口密度以及各个国家的汽车拥有量计算出格点内的汽车数量，
然后根据汽车行驶排放的热量以及交通流量的日分布计算出不同时刻的交通热排放。
其数据来源如表~\ref{tab:交通热数据及来源}，具体计算方法如下：
\begin{equation}
Q_{v}=\frac{\left(V_{c} E_{c}+V_{M} E_{M}+V_{F R} E_{F R}\right)  \cdot P \cdot H_{t r a c} \cdot D}{3600 \cdot 10^{6}}
\end{equation}
% Please add the following required packages to your document preamble:
% \usepackage{booktabs}
\begin{table}[]
...
\label{tab:交通热数据及来源}
    ...
    \end{tabular}
    \end{table}
其中$V_c$、$V_M$、$V_{FR}$分别为每千人拥有的汽车、摩托车、货车(巴士)的数量，$H_{trac}$为时间步长内交通流量分布，以上数据采用 \citet{allen2011}；
$E_c$、$E_M$、$E_{FR}$则为三种机动车的排放系数，目前使用 \citet{sailor2004top} 的系数设置(3975J/m)；
...
\begin{figure}[h!]
\centering
\includegraphics[width=0.8\textwidth]{Figures/城市模式/CoLM城市模式人为热阻抗交换网络.png}
\caption{考虑人为热时城市内部湍流交换过程}
\label{fig:考虑人为热时城市内部湍流交换过程}
\end{figure}
}

考虑人为热时，城市内部湍流过程示意图 \ref{fig:考虑人为热时城市内部湍流交换过程} 所示，在建筑群落中，不同的城市组成部分在不同层次进行交换，其中，与第一层($layer_{1}$)直接交换的有地面(透水面与不透水面)、前面(阳面墙与阴面墙)、植被以及部分在墙面排放的人为热，屋顶与屋顶释放的人为热则与第二层($layer_{2}$)直接交换，层与层交换总是守恒的。因此，对于感热而言，其不同层次的感热通量$H$可表达为：
\begin{equation}\label{2lays_H_layer1}
    H_{layer_{1}} = H_{g}
\end{equation}
\begin{equation}\label{2lays_H_layer2}
    H_{layer_{2}} = H_{g} + H_{wsun} + H_{wsha} + H_{veg} + H_{whac} = \frac{\rho \cdot c \cdot \left( T_{layer_{1}} - T_{layer_{2}} \right)}{r_{d}}
\end{equation}
\begin{equation}\label{2lays_H}
    H = H_{layer_{2}} + H_{roof} + H_{rhac} = \frac{\rho \cdot c \cdot \left( T_{layer_{2}} - \theta \right)}{r_{ah}}
\end{equation}
其中，$H_{g}$、$H_{wsun}$、$H_{wsha}$、$H_{veg}$、$H_{roof}$分别为地面、阳面墙、阴面墙、植被以及屋顶的感热通量，$H_{whac}$、$H_{rhac}$为墙面以及屋顶释放的建筑热，其各自的表达式分别为：
\begin{equation}
    H_{g} = \frac{\rho \cdot c \cdot \left( T_{g} - T_{layer_{1}} \right) \cdot f_{g}}{r_{gh}} + H_{vehc} + H_{humMeta}
\end{equation}
\begin{equation}
    H_{wsun} = \frac{\rho \cdot c \cdot \left( T_{wsun} - T_{layer_{1}} \right) \cdot fc(1)}{r_{wsun}}
\end{equation}
\begin{equation}
    H_{wsha} = \frac{\rho \cdot c \cdot \left( T_{wsha} - T_{layer_{1}} \right) \cdot fc(2)}{r_{wsha}}
\end{equation} 
\begin{equation}
    H_{veg} = \frac{\rho \cdot c \cdot \left( T_{veg} - T_{layer_{1}} \right) \cdot fc(3)}{r_{t}}
\end{equation}
\begin{equation}
    H_{roof} = \frac{\rho \cdot c \cdot \left( T_{roof} - T_{layer_{2}} \right) \cdot fc(0)}{r_{roof}}
\end{equation}
由于代谢热与交通热($H_{humMeta}$ 、$H_{vehc}$)都在地面释放，因此我们将其纳入地面感热$H_{g}$，其中$T_{g}$为透水面与不透水面温度加权得到的平均温度，$r$为空气动力学阻抗，$fc(0-3)$分表代表屋顶、阳面墙、阴面墙以及植被的覆盖比例，$f_{g}$则为除去建筑覆盖外的路面占比(包含透水面$f_{gper}$以及不透水面$f_{gimp}$)，将以上表达式代回式 \ref{2lays_H_layer1} $\sim$ \ref{2lays_H}，可得：
\begin{equation}
    \begin{split}
        & \frac{\rho \cdot c \cdot \left( T_{layer_{1}} - T_{layer_{2}} \right)}{r_{d}} = \\
        & \frac{\rho \cdot c \cdot \left( T_{g} - T_{layer_{1}} \right) \cdot f_{g}}{r_{gh}} + H_{vehc} + H_{humMeta} + \frac{\rho \cdot c \cdot \left( T_{wsun} - T_{layer_{1}} \right) \cdot {fc}\left( 1 \right)}{r_{wsun}} + \\
        & \frac{\rho \cdot c \cdot \left( T_{wsha} - T_{layer_{1}} \right) \cdot fc\left( 2 \right)}{r_{wsha}} + \frac{\rho \cdot c \cdot \left( T_{veg} - T_{layer_{1}} \right) \cdot {fc}\left( 3 \right)}{r_{t}} + H_{whac}
    \end{split}
\end{equation}
\begin{equation}
    \begin{split}
        & \frac{\rho \cdot c \cdot \left( T_{layer_{2}} - \theta \right)}{r_{ah}} = \\
        & \frac{\rho \cdot c \cdot \left( T_{layer_{1}} - T_{layer_{2}} \right)}{r_{d}} + \frac{\rho \cdot c \cdot \left( T_{roof} - T_{layer_{2}} \right)*fc\left( 0 \right)}{r_{roof}} + H_{rhac}
    \end{split}
\end{equation}
进而得到关于$T_{layer1}$和$T_{layer2}$的二元一次方程：
\begin{equation}
    \begin{split}
        & T_{layer_{1}} = \\
        & \frac{\frac{T_{g} \cdot f_{g}}{r_{gh}} + \frac{H_{vehc} + H_{humMeta} + H_{whac}}{\rho \cdot c} + \frac{T_{wsun} \cdot fc\left( 1 \right)}{r_{wsun}} + \frac{T_{wsha} \cdot fc\left( 2 \right)}{r_{wsha}} + \frac{T_{veg} \cdot fc\left( 3 \right)}{r_{t}} + \frac{T_{layer_{2}}}{r_{d}}}{\frac{1}{r_{d}} + \frac{f_{g}}{r_{gh}} + \frac{fc\left( 1 \right)}{r_{wsun}} + \frac{fc\left( 2 \right)}{r_{wsha}} + \frac{fc\left( 3 \right)}{r_{t}}}
    \end{split}
\end{equation}
\begin{equation}\label{result_T_layer2}
    T_{layer_{2}} = \frac{\frac{T_{layer_{1}}}{r_{d}} + \frac{T_{roof} \cdot fc\left( 0 \right)}{r_{roof}} + \frac{H_{rhac}}{\rho \cdot c} + \frac{\theta}{r_{ah}}}{r_{d} \cdot \left(\frac{1}{r_{ah}} + \frac{1}{r_{d}} + \frac{fc\left( 0 \right)}{r_{roof}} \right)}
\end{equation}
求解该二元一次方程可知：
\begin{equation}
    \begin{split}
    % \hspace{-1cm}
        & T_{layer_{1}} = \\
        & \frac{\frac{T_{g} \cdot f_{g}}{r_{gh}} + \frac{H_{vehc} + H_{humMeta} + H_{whac}}{\rho \cdot c} + \frac{T_{wsun} \cdot fc\left( 1 \right)}{r_{wsun}} + \frac{T_{wsha} \cdot fc\left( 2 \right)}{r_{wsha}} + \frac{T_{veg} \cdot fc\left( 3 \right)}{r_{t}} + a}{c \cdot \left( 1 - \frac{b}{r_{d} \cdot c} \right)}
    \end{split}
\end{equation}

\begin{equation}
    a = \left(\frac{T_{roof} \cdot fc\left( 0 \right)}{r_{roof}} + \frac{H_{rhac}}{\rho \cdot c} + \frac{\theta}{r_{ah}}\right) \cdot b
\end{equation}
\begin{equation}
    b = \frac{1}{r_{d} \cdot \left(\frac{1}{r_{ah}} + \frac{1}{r_{d}} + \frac{fc\left( 0 \right)}{r_{roof}} \right)}
\end{equation}
\begin{equation}
    c = \frac{1}{r_{d}} + \frac{f_{g}}{r_{gh}} + \frac{fc\left( 1 \right)}{r_{wsun}} + \frac{fc\left( 2 \right)}{r_{wsha}} + \frac{fc\left( 3 \right)}{r_{t}}
\end{equation}
将$T_{layer_{1}}$代回式\ref{result_T_layer2}，即可求得$T_{layer_{2}}$。

对于潜热通量而言，其求解过程与感热通量的计算类似，但是在计算潜热通量时，由于墙体难以表达积水过程，因此认为墙体没有潜热交换，并且人为热目前全部作为感热释放，因此，对于潜热通量而言，在第一层发生交换的仅有地面、植被，并且由于考虑土壤阻抗，透水面与不透水面需要单独计算，在第二层发生交换的仅有屋顶，对于不透水面与屋顶，仅考虑其淹没部分($fwet$)，所以对于建筑群落内部不同层次的潜热通量计算，其表达式为：
\begin{equation}\label{2lays_L_layer1}
    L_{layer_{1}} = L_{g}
\end{equation}
\begin{equation}
    L_{layer_{2}} = L_{g} + L_{veg}
\end{equation}
\begin{equation}\label{2lays_L}
    L = L_{layer_{2}} + L_{roof}
\end{equation}
其中，$L_{g}$、$L_{veg}$、$L_{roof}$分别为地面、植被以及屋顶的潜热通量，其表达式分别为：
\begin{equation}
    L_{g} = \frac{\rho \cdot \left( q_{gper}-q_{layer_{1}} \right) \cdot f_{g} \cdot f_{gper}}{r_{gw}+rss} + \frac{\rho \cdot \left( q_{gimp}-q_{layer_{1}} \right) \cdot fwet_{gimp} \cdot f_{g} \cdot f_{gimp}}{r_{gw}}
\end{equation}
\begin{equation}
    L_{veg} = \frac{\rho \cdot \left( q_{veg}-q_{layer_{1}}\right) \cdot fc\left( 3 \right)}{r_{t}}
\end{equation}
\begin{equation}
    L_{roof} = \frac{\rho \cdot \left( q_{roof}-q_{layer_{2}}\right) \cdot fwet_{roof} \cdot fc\left( 0 \right)}{r_{roof}}
\end{equation}
由于城市中的地面可以分为透水面和不透水面，对于透水面，因为其本质是土壤，因此在计算其潜热通量时，考虑土壤阻抗(rss)，$fwet$表示各部分干湿的比例，q为不同表面的比湿，将以上表达式代回\ref{2lays_L_layer1} $\sim$ \ref{2lays_L}，可知：
\begin{equation}
   \begin{split}
    & \frac{\rho \cdot \left( q_{layer_{1}}-q_{layer_{2}}\right)}{r_{d}} = \\
    & \frac{\rho \cdot \left( q_{gper}-q_{layer_{1}}\right) \cdot f_{g} \cdot f_{gper}}{r_{gw}+rss} + \frac{\rho \cdot \left( q_{gimp}-q_{layer_{1}} \right) \cdot fwet_{gimp} \cdot f_{g} \cdot f_{gimp}}{r_{gw}} + \\
    &\frac{\rho \cdot \left( q_{veg}-q_{layer_{1}}\right) \cdot fc\left( 3 \right)}{r_{t}}
   \end{split} 
\end{equation}

\begin{equation}
    \frac{\rho \cdot \left( q_{layer_{2}} - Q\right)}{r_{aw}} = \frac{\rho \cdot \left( q_{layer_{1}} - q_{layer_{2}}\right)}{r_{d}} + \frac{\rho \cdot \left( q_{roof}-q_{layer_{2}}\right) \cdot fwet_{roof} \cdot fc\left( 0 \right)}{r_{roof}}
\end{equation}
同样可得关于$q_{layer_{1}}$以及$q_{layer_{2}}$的二元一次方程：
\begin{equation}
    q_{layer_{1}} = \\
    \frac{\frac{q_{gper} \cdot f_{g} \cdot f_{gper}}{r_{gw} + rss} + \frac{q_{gimp} \cdot f_{g} \cdot fwet_{gimp} \cdot f_{gimp}}{r_{gw}} + \frac{q_{veg} \cdot fc\left( 3 \right)}{r_{t}} + \frac{q_{layer_{2}}}{r_{d}}}{\frac{1}{r_{d}} + \frac{f_{g} \cdot f_{gper}}{r_{gw} + rss} + \frac{fwet_{gimp} \cdot f_{g} \cdot f_{gimp}}{r_{gw}}+\frac{fc\left(3\right)}{r_{t}}}
\end{equation}
\begin{equation}\label{equ_2lays_Q_lay2}
    q_{layer_{2}} = \frac{\frac{q_{layer_{1}}}{r_{d}} + \frac{q_{roof} \cdot fwet_{roof} \cdot fc\left( 0 \right)}{r_{roof}} + \frac{Q}{r_{aw}}}{\frac{1}{r_{aw}} + \frac{1}{r_{d}} + \frac{fwet_{roof} \cdot fc\left( 0 \right)}{r_{roof}}}
\end{equation}
求解可得：
\begin{equation}
    q_{layer_{1}} = \\
    \frac{\frac{q_{gper} \cdot f_{g} \cdot f_{gper}}{r_{gw}+rss} + \frac{q_{gimp} \cdot f_{g} \cdot fwet_{gimp} \cdot f_{gimp}}{r_{gw} } + \frac{q_{veg} \cdot fc\left( 3 \right)}{r_{t}} + a}{c \cdot \left( 1 - \frac{b}{r_{d} \cdot c}\right)}
\end{equation}

\begin{equation}
    a = \left(\frac{q_{roof} \cdot fc\left(0\right)}{r_{roof}} + \frac{Q}{r_{aw}}\right) \cdot b
\end{equation}
\begin{equation}
    b = \frac{1}{r_{d} \cdot \left( \frac{1}{r_{aw}} + \frac{1}{r_{d}} + \frac{fc\left(0\right)}{r_{roof}} \right)}
\end{equation}
\begin{equation}
    c = \frac{1}{r_{d}}+\frac{f_g \cdot f_{gper}}{r_{gw}+rss} + \frac{fwet_{gimp} \cdot f_g \cdot f_{gimp}}{r_{gw}} + \frac{fc\left( 3 \right)}{r_t}
\end{equation}
同理，将$q_{layer_{1}}$带入式\ref{equ_2lays_Q_lay2}即可求得$q_{layer_{2}}$。