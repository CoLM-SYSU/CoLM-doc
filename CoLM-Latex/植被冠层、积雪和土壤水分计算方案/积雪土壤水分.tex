...
\end{equation}
$w_{\mathrm {a}} $的绝对值表示土壤水计算区域之下的空气的总量（单位为：单位面积上的体积），负号表示计算区域之下液态水相对于饱和状态是亏缺的，$w_{\mathrm {a}} $的最大值为0（表示土壤水饱和）。\todo{bot含义是否需要说明}
...
\begin{enumerate}
  ...
  \item 相邻异质不饱和土壤层之间的水流通量，使用公式
    ...
    $k_{\mathrm{hm}}$为等效水力导度，可变饱和流算法中采用带权重的几何平均，分为三种情况进行计算，对入渗情形($h_{\mathrm {u}} >h_{\mathrm {l}} $)，\todo{是否需要说明ku和kl的含义}
    \begin{equation}
...
\begin{equation} \label{formula:prediction_zwt}
  ...
  \end{aligned}
\end{equation}
其中，$z_{\mathrm{wt}}^{\mathrm{old}}$为目前的地下水位，$z_{\mathrm{wt}}^{\mathrm{new}}$为预估的地下水位，$\theta_{\mathrm{sat},i}$为饱和含水量，$\psi_{\mathrm{sat},i}$为饱和时的土壤水势，$\Theta_i$表示第$i$层土壤内由土壤水势到土壤含水量的映射。式~\eqref{formula:prediction_zwt}为$z_{\mathrm{wt}}^{\mathrm{new}}$的隐函数，可用迭代方法求解。\todo{$\theta_{pre}$和$\Delta W$是否需要说明变量含义}
