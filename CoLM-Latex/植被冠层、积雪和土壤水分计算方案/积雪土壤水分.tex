...
当无积雪时，
\begin{equation}
  \Delta W_{\mathrm{ponding}} = P_{\mathrm{rain}}+S_{\mathrm{melt}}+Q_{\mathrm{dew}}-E-R-Q_{\mathrm{infl}}
\end{equation}
其中$\Delta W_{\mathrm{ponding}}$为地表积水的变化，$P_{\mathrm{rain}}$为经过植被截留后到达地面的降水，$S_{\mathrm{melt}}$为积雪融化后的水分，$Q_{\mathrm{dew}}$为凝结的水分，$E$为土壤表面的蒸发，$R$为地表产流，$Q_{\mathrm{infl}}$为入渗到土壤中的水分。

当有积雪时，
\begin{equation}
  \Delta W_{\mathrm{ponding}} = S_{\mathrm{melt}}-R-Q_{\mathrm{infl}}
\end{equation}
其中$\Delta W_{\mathrm{ponding}}$为地表积水的变化，$S_{\mathrm{melt}}$为由积雪空隙到达地表的液态水，$R$为地表产流，$Q_{\mathrm{infl}}$为入渗到土壤中的水分。

积雪中的液态水平衡为，
\begin{equation}
  \Delta W_{\mathrm{snow}} = P_{\mathrm{rain}}+Q_{\mathrm{dew}}-S_{\mathrm{melt}}-E
\end{equation}
其中$\Delta W_{\mathrm{snow}}$为积雪空隙中液态水的变化，$P_{\mathrm{rain}}$为经过植被截留后到达地面的降水，$Q_{\mathrm{dew}}$为凝结的水分，$S_{\mathrm{melt}}$为由积雪空隙到达地表的液态水，$E$为积雪表面的蒸发。
...
\begin{equation}\label{eq:SW_CB}
  \psi=\psi_{\mathrm{s}}\left(\frac{\theta}{\theta_{\mathrm{sat}}}\right)^{-B}
\end{equation}
\begin{equation}\label{eq:Ks_CB}
  K=K_{\mathrm{s}}\left(\frac{\theta}{\theta_{\mathrm{s}}}\right)^{2 B+3}
\end{equation}
其中$\psi_{\mathrm {s}} $为饱和土水势，$\theta_{\mathrm{s}}$为饱和土壤体积含水量，$K_{\mathrm{s}}$为饱和导水率，$B$为曲线参数。

\citet{van1980closed} 建立的曲线为
\begin{equation}\label{eq:SW_VG}
  \Theta = \frac{\theta-\theta_{\mathrm {r}} }{\theta_{\mathrm {s}} -\theta_{\mathrm {r}} } = \left[\frac{1}{1+\left(\alpha h\right)^n}\right]^{1-1/n}
\end{equation}
\begin{equation}\label{eq:Ks_VG}
  K = K_{\mathrm {s}}  \Theta^L \left[1-\left(1-\Theta^{1/\left(1-1/n\right)}\right)^{1-1/n}\right]^2
\end{equation}
其中$h$为土壤吸力势，$\theta_{\mathrm {r}} $为残余含水量，$\theta_{\mathrm {s}} $为饱和含水量，$K_{\mathrm{s}}$为饱和导水率，$\alpha$、$L$ 和 $n$为曲线参数。
...
\begin{equation}
  K_{i+\frac{1}{2}}=
  \begin{cases}
    K_{\mathrm{s},i}\left(\frac{\theta_{i}}{\theta_{\mathrm{s},i}}\right)^{2 B_i+3}
    & q_{i+\frac{1}{2}} \geqslant 0 \quad \mbox{（水流方向向下）}\\
    K_{\mathrm{s},i +1}\left(\frac{\theta_{i+1}}{\theta_{\mathrm{s},i +1}}\right)^{2 B_{i+1}+3}
...
\begin{align}
  ...
  \begin{cases}
    \left(2B_i+3\right) K_{\mathrm{s},i}\left(\frac{\theta_{i}}{\theta_{\mathrm{s},i}}\right)^{2B_i+2}
    \frac{1}{\theta_{\mathrm{s},i}}, & q_{i+\frac{1}{2}} \geqslant 0 \\
    0, & q_{i+\frac{1}{2}} < 0
  \end{cases}
  \\
  ...
  \begin{cases}
    ...
    (2B_{i+1}+3) K_{\mathrm{s},i +1}\left(\frac{\theta_{i+1}}{\theta_{\mathrm{s},i +1}}\right)^{2B_{i+1}+2} \frac{1}{\theta_{\mathrm{ {s},i +1}}}, & q_{i+\frac{1}{2}} < 0
  \end{cases}
\end{align}
对计算区域的底部（第$I$层），
\begin{equation}
  K_{I}=K_{\mathrm{s},I}\left(\frac{\theta_{I}}{\theta_{\mathrm{s},I}}\right)^{2B_I+3}, \quad \frac{\partial K_{I}}
  {\partial \theta_{I}} = (2B_I+3) K_{\mathrm{s},I}\left(\frac{\theta_{I}}{\theta_{\mathrm{s},I}}\right)^{2B_I+2} \frac{1}{\theta_{\mathrm{s},I}}
\end{equation}
...
\begin{equation}
  \psi \left(z\right) = \psi_{\mathrm {s}}  - \left(z_{\mathrm{wt}} - z\right)
\end{equation}
地下水位之上的土壤中所含水分的总量为
\begin{equation}
  w_{\mathrm{unsat}} = \int^{z_{\mathrm{wt}}}_0 \theta\left[\psi\left(z\right)\right] \mathrm{d}z = \int^{z_{\mathrm{wt}}}_0 \theta\left[\psi_{\mathrm {s}}  - \left(z_{\mathrm{wt}} - z\right)\right] \mathrm{d}z
\end{equation}
使用~\citet{campbell1974}建立的土壤水力特征曲线时，
\begin{equation}
  w_{\mathrm{unsat}} = \int^{z_{\mathrm{wt}}}_0 \theta_{\mathrm {s}} \left[\frac{\psi_{\mathrm {s}}  - \left(z_{\mathrm{wt}} - z\right)}{\psi_{\mathrm {s}} }\right]^{-\frac{1}{B}} \mathrm{d}z
\end{equation}
假设饱和含水层最大到$z_{\mathrm{max}}$，则地下水位之下的水分总量为
\begin{equation}
  w_{\mathrm{sat}} = \theta_{\mathrm {s}} \left(z_{\mathrm{max}}-z_{\mathrm{wt}}\right)
\end{equation}
土壤$0$到$z_{\mathrm{max}}$深度的总水量为
\begin{equation}
  w_{\mathrm{total}} = w_{\mathrm{unsat}} + w_{\mathrm{sat}} = \int^{z_{\mathrm{wt}}}_0 \theta_{\mathrm {s}} \left[\frac{\psi_{\mathrm {s}}  - \left(z_{\mathrm{wt}} - z\right)}{\psi_{\mathrm {s}} }\right]^{-\frac{1}{B}} \mathrm{d}z + \theta_{\mathrm {s}} \left(z_{\mathrm{max}}-z_{\mathrm{wt}}\right)
\end{equation}
对$z_{\mathrm{wt}}$求导数可得给水度$S_{\mathrm {y}} $，
\begin{equation}
  {S}_{\mathrm{{y}}} = -\frac{w_{\mathrm{total}}}{z_{\mathrm{wt}}} = \theta_{\mathrm{s}}\left[1-\left(1-\frac{z_{\mathrm{w t}}}{\psi_{\mathrm{s}}}\right)^{-\frac{1}{B}}\right] \label{eqn:s_y}
\end{equation}
...
若地下水位位于最下层土壤之下，公式（\ref{eqn:s_y}）中的参数$\theta_{\mathrm {s}} $、$\psi_{\mathrm {s}} $ 和 $B$取最下层土壤的值，则水位的变化为
\begin{equation}
...
\begin{equation}
  {S}_{\mathrm{y},j}=\theta_{\mathrm{s},j}\left[1-\left(1-\frac{z_{\mathrm{wt}}}{\psi_{\mathrm{s},j}}\right)^{-\frac{1}{B_j}}\right]
\end{equation}
...
\begin{equation}
  r_{\mathrm{subsurface}},j = \min \left[r_{\mathrm{subsurface}}, \frac{S_{\mathrm{y},j} \cdot \left(z_{j+\frac{1}{2}} - z_{\mathrm{w t}}\right)}{\Delta t} \right]
\end{equation}
其中$S_{\mathrm{y},j}$为第$j$层内的给水度，由~\eqref{eqn:s_y}计算。地下水位在第$j$层内的变化为
\begin{equation}
  z_{\mathrm{w t}}=z_{\mathrm{w t}}+\frac{r_{\mathrm{subsurface},j} \cdot \Delta t}{S_{\mathrm{y},j}}
\end{equation}
土壤水的变化为
\begin{equation}
...
若地下水位降到了最下层土壤之下，公式~\eqref{eqn:s_y}中的参数$\theta_{\mathrm {s}} $、$\psi_{\mathrm {s}} $ 和 $B$取最下层土壤的值，则含水层总水量和水位的变化为
\begin{equation}
  \begin{aligned}
    w_{\mathrm{a}} & = w_{\mathrm{a}} - r_{\mathrm{subsurface}} \cdot \Delta t, \\
    z_{\mathrm{wt}} & = z_{\mathrm{w t}}+\frac{r_{\mathrm{subsurface}} \cdot \Delta t}{S_{\mathrm{y}}}.
  \end{aligned}
\end{equation}
...
\begin{equation}\label{si_in2}
  \left(\theta_{\mathrm{s},i}-\theta_{i}^{n}\right) \cdot\left(w_{\mathrm{f},i}^{n+1}-w_{\mathrm{f},i}^{n}\right)=\Delta t \cdot\left(q_{i-1/2}^{n+1}-q_{\mathrm{w f},i}^{n+1}\right)
\end{equation}
其中，$\theta_{\mathrm{s},i}$ 为第$ i$ 层土壤的饱和体积含水量，$ q_{{i-1/2}}^{n+1}$为第$i$层土壤上边界处的水流通量，$q_{\mathrm{wf},i}^{n+1}$为饱和区域下边界处（湿润锋面）的水流通量。


预报变量$w_{\mathrm{t},i}$表示土壤层下部饱和区域的厚度，其离散后的预报方程为
\begin{equation}\label{si_in3}
  \left(\theta_{\mathrm{s},i}-\theta_{i}^{n}\right) \cdot\left(w_{\mathrm{t},i}^{n+1}-w_{\mathrm{t},i}^{n}\right)=\Delta t \cdot\left(q_{\mathrm{w t},i}^{n+1}-q_{i+1 / 2}^{n+1}\right)
\end{equation}
...
\begin{equation}\label{si_in4}
  \begin{aligned}
    m_{\mathrm{total}}^n & \triangleq \theta_{\mathrm{s},i}\left(w_{\mathrm{f},i}^n+w_{\mathrm{t},i}^{n}\right)+\theta_{i}^{n}\left(\Delta z - w_{\mathrm{f},i}^{n} - w_{\mathrm{t},i}^{n}\right) \\
...
\begin{equation}
  w_{\mathrm{a}}=-\int_{\mathrm{z_{\mathrm{btm}}}}^{z_{\mathrm{wt}}}\left(\theta_{\mathrm{s}}-\theta\right){\mathrm { d}} z
\end{equation}
...
\begin{enumerate}
  \item 饱和区域内部的水流通量，使用公式
    \begin{equation}\label{q_sat1}
      q_{\mathrm{sat}}=-\frac{\sum_{i=i_{1}}^{i_{2}} \Delta z_{i}}{\sum_{i=i_{1}}^{i_{2}} \frac{\Delta z_{i}}{K_{\mathrm{s},i}}}
    ...
    $K_{\mathrm{s},i}$为第$i$层的饱和土壤导水率，$h_{\mathrm {l}} $为饱和区域下边界处的土壤水势。
  ...
  \item 饱和区位于非饱和区上方时两者之间的水流通量，即湿润锋面处的水流通量，公式为
    \begin{equation}
      q_{\mathrm{wf}}=q_{\mathrm{h m}}\left(z_{\mathrm{l}}-z_{\mathrm{i}}, h_{\mathrm{s}}, h_{\mathrm{l}}\right)
    \end{equation}
    其中，$z_{\mathrm {l}} $为下方非饱和土壤层中心点的位置，$z_i$为饱和区和非饱和区之间界面的位置，
    $h_{\mathrm {s}} $为饱和土壤水势，$h_{\mathrm {l}} $为非饱和土壤层中心点处的土壤水势。

  \item 饱和区位于非饱和区上方时两者之间的水流通量，即饱和水位处的水流通量，公式为
    \begin{equation}
      q_{\mathrm{wt}}=q_{\mathrm{h m}}\left(z_{\mathrm{i}}-z_{\mathrm{u}}, h_{\mathrm{u}}, h_{\mathrm{s}}\right)
    \end{equation}
    其中，$z_{\mathrm {u}} $为上方非饱和土壤层中心点的位置，$z_i$为饱和区和非饱和区之间界面的位置，
    $h_{\mathrm {s}} $为饱和土壤水势，$h_{\mathrm {u}} $为非饱和土壤层中心点处的土壤水势。
...
\begin{equation}\label{richards_nls}
  \begin{aligned}
    ...
    & \theta_{\mathrm{r},i}<\theta_{\mathrm{i}} \leqslant \theta_{\mathrm{s},i}, & \forall \theta_{i} \in \vec{x} \\
...
这里对土壤水方程中的源汇项$S$的主要来源，即根系吸水作用(蒸腾)过程做一简单介绍。植物吸水作用由有效根比例$f_{\mathrm{root},i}$和叶面蒸腾$E_{\mathrm{tr}}$的乘积来表示\citep{dai2003common}。有效根比例为：
\begin{equation}
  {f}_{\mathrm{ {root }}, {i}}=\frac{{f}_{\mathrm{{root}}, {i}} {W}_{\mathrm{{LT}}}[{i}]}{W_{\mathrm{t}}}
\end{equation}
其中$W_{\mathrm{t}} = \sum_{i=1}^{n}{f_{\mathrm{root,i\ }}W_{\mathrm{LT}}\left[i\right]}$；$f_{\mathrm{root},i}$表示模式中第$i$层土壤中的根系比例；$W_{\mathrm{LT}}[i]$表示第$i$层土壤中的水分胁迫状况：
\begin{equation}
  {W}_{\mathrm{{LT}}}[{i}]=\frac{\psi_{\max }-\psi_{i}}{\psi_{\max }-\psi_{\mathrm{sat}}}
\end{equation}
其中，$\psi_{\mathrm{max}}$为植物达到萎蔫点时的土壤水势。

叶面蒸腾表示为：
\begin{equation}
  {E}_{\mathrm{{tr}}}=\sigma_{\mathrm{{f}}} {\rm SAI} \delta\left({E}_{\mathrm{{f}}}^{{\rm pot}}\right) {L}_{\mathrm{{d}}} \frac{{r}_{\mathrm{{b}}}}{{r}_{\mathrm{{b}}}-{r}_{\mathrm{{s}}}} {E}_{\mathrm{{f}}}^{{\rm pot}} \leqslant {E}_{\mathrm{{trmax}}}
\end{equation}
其中，$\sigma_{\mathrm {f}} $表示没有被雪覆盖的植物比例，${\rm SAI}$表示叶面积指数和茎面积指数之和，$\delta\left(E_{\mathrm {f}} ^{\rm pot}\right)$为表示蒸发是否发生的参数，
取值0或1，$L_{\mathrm {d}} $为干燥的植物表面比例，$r_{\mathrm {b}} $为叶片边界层阻抗，$r_{\mathrm {s}} $为叶片气孔阻抗。$E_{\mathrm {f}} ^{\rm pot}$表示植物表面水的单位面积蒸发量：
\begin{equation}
...
对于其他参数，CoLM模式研发团队针对~\citet{campbell1974}和~\citet{van1980closed}建立的土壤水力特征曲线关系中包含的参数，收集了超过30种较为常用或新近开发的土壤转换函数模型(PTF)（见表~\ref{tab:PTFs}），采用拟合所有PTF对应的土壤水力特征曲线关系的方式获取最优土壤水力特征曲线关系，从而得到最优关系下的土壤水力参数。以~\citet{campbell1974}土壤水力特征曲线关系中参数的计算为例，两个待定参数$\psi_{\mathrm {s}} $和$B$通过求解下列极值问题得到：$$\chi\left(\psi_{\mathrm {s}} ,B\right)=\min\sum_{i=1}^N\left[\theta\left(\psi_{\mathrm {s}} ,B\right)-\theta_i\left(\psi_{\mathrm{si}},B_{i}\right)\right]^2$$
其中，$\psi_{\mathrm{si}}$和$B_{i}$为每一组PTF对应的参数。通过此方法得到的土壤水力特征曲线关系最为接近PTF集合内每一组PFT得到的土壤水力特征曲线关系，因此其对应的参数$\psi_{\mathrm{s}}$和$B$可视为PFT集合内的最优参数。\citet{van1980closed}土壤水力特征曲线关系中的参数同理可得。\citet{dai2019parameters}通过与NCSS提供的土壤水分特征曲线的观测数据进行比对，发现基于最优拟合参数的~\citet{campbell1974}和~\citet{van1980closed}两种土壤水力特征曲线关系对土壤含水量的模拟效果精度较高，且最优拟合参数相较于传统的PTF中位值参数针对土壤水力特征曲线关系对土壤含水量的模拟结果具有一定程度上的改进。
...
%\item 注：
    ...
    \item[2] “提供参数种类”一项中，Campbell代表该土壤转换函数提供~\citet{campbell1974}土壤水力特征曲线关系中包含的参数，VG代表该土壤转换函数提供~\citet{van1980closed}土壤水力特征曲线关系中包含的参数，$K_{\mathrm {s}} $代表饱和土壤导水率
    \end{TableNotes}
...
%\bottomrule
        ...
        Cosby0       & \citet{cosby1984statistical}    & 1343 & Campbell, $K_{\mathrm {s}} $     & + &   &   &   \\ \hline
        Cosby1       & \citet{cosby1984statistical}    & 1343 & Campbell, $K_{\mathrm {s}} $     &   & + &   &   \\ \hline
        Cosby2       & \citet{cosby1984statistical}    & 1343 & Campbell, $K_{\mathrm {s}} $     &   & + &   &   \\ \hline
        Campbell1    & \citet{campbell_Shiozawa_1992}  & 237  & Campbell                         &   & + & + &   \\ \hline
        Rawls1       & \citet{Rawls_Brakensiek_1989}   & 572  & Campbell, VG, $K_{\mathrm {s}} $ &   & + & + & + \\ \hline
        Mayr         & \citet{Mayr_Jarvis_1999}        & 114  & Campbell                         &   & + & + & + \\ \hline
        Williams     & \citet{Williams_Ross_1992}      & 256  & Campbell                         &   & + & + &   \\ \hline
        Clapp        & \citet{clapp1978empirical}      & 2378 & Campbell, $K_{\mathrm {s}} $     & + &   &   &   \\ \hline
        Carsel       & \citet{Carsel_Parrish_1988}     & 1801 & VG                               & + &   &   &   \\ \hline
        Wosten1      & \citet{Wösten_Lilly_1999}       & 911  & VG, $K_{\mathrm {s}} $           &   & + & + & + \\ \hline
        Wosten2      & \citet{Wösten_Lilly_1999}       & 911  & VG, $K_{\mathrm {s}} $           & + & + &   &   \\ \hline
        Weynants     & \citet{Weynants_Vereecken_2009} & 96   & VG, $K_{\mathrm {s}} $           &   & + &   & + \\ \hline
        Rosetta3-H1w & \citet{Zhang_Schaap_2017}       & 1638 & VG                               & + &   &   &   \\ \hline
        Rosetta3-H3w & \citet{Zhang_Schaap_2017}       & 1638 & VG, $K_{\mathrm {s}} $           &   & + & + &   \\ \hline
        ...
        Ahuja        & \citet{Ahuja_1989}              & 231  & $K_{\mathrm {s}} $               &   & + & + & + \\ \hline
        Suleiman     & \citet{Suleiman_2001}           & 61   & $K_{\mathrm {s}} $               &   & + & + & + \\ \hline
        Spychalski   & \citet{Spychalski_2007}         & 7    & $K_{\mathrm {s}} $               &   & + & + & + \\ \hline
        Dane         & \citet{Dane_Puckett_1994}       & 256  & $K_{\mathrm {s}} $               &   & + &   &   \\ \hline
        Jabro        & \citet{Jabro_1992}              & 183  & $K_{\mathrm {s}} $               &   & + & + &   \\ \hline
        Brakensiek   & \citet{Brakensiek_1984}         & -    & $K_{\mathrm {s}} $               &   & + & + & + \\ \hline
        Julia        & \citet{Julia_2004}              & 69   & $K_{\mathrm {s}} $               &   & + &   &   \\ \hline
        Campbell2    & \citet{Campbell_1985}           & -    & $K_{\mathrm {s}} $               &   & + & + &   \\ \hline
        Vereecken    & \citet{Vereecken_Maes_1990}     & 771  & $K_{\mathrm {s}} $               &   & + & + & + \\ \hline
        Merdun1      & \citet{Merdun_2010}             & 12   & $K_{\mathrm {s}} $               &   & + & + & + \\ \hline
        Merdun2      & \citet{Merdun_2010}             & 12   & $K_{\mathrm {s}} $               &   & + & + & + \\ \hline
        Aimrun       & \citet{Aimrun_2009}             & 23   & $K_{\mathrm {s}} $               &   & + & + & + \\ \hline
        Rahmati1     & \citet{Rahmati_2018}            & 5    & $K_{\mathrm {s}} $               & + &   &   &   \\ \hline
        Rahmati2     & \citet{Rahmati_2018}            & 5    & $K_{\mathrm {s}} $               & + &   &   &   \\
%\hline
      \end{longtable}
    \end{center}
  \end{ThreePartTable}
