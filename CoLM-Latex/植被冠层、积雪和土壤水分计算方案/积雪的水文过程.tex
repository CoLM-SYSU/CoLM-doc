...
\end{equation}
其中$p_{\mathrm {i}} $为固态降水速率(\unit{kg.m^{-2}.s^{-1}})，$\Delta t$为积分时间步长(s)；在水热过程模拟结束后，若有积雪融化发生，则$f_{\mathrm{sno}}$更新为
\begin{equation}
  f_{\mathrm{sno}}^{(n+1)}=\tanh{\left(\frac{100 z_{\mathrm{s n o}}^{2}}{2.5 z_{0 m, s o i} W_{\mathrm{sno}}}\right)}
\end{equation}
其中$W_{\mathrm{sno}}$为雪水当量(mm)，$z_{0m,soi}$ 为未被积雪覆盖时的地表粗糙度。

当植被被积雪掩埋时，已知植被粗糙度$z_{0mv}$，则被积雪掩埋的植被占总植被的比例计算为
\begin{equation}
  wt=\frac{0.1 z_{\mathrm{{sno}}}}{z_{0mv}+0.1 z_{\mathrm{{sno}}}}
\end{equation}
...
当固态降水$p_{\mathrm{i}}$的发生导致雪盖高度$z_{\mathrm{sno}}$大于0.01 m且此时尚无雪盖分层，则将在模拟开始时创建一个新的雪层，相关物理量设置如下：
\begin{equation}
  \begin{aligned}
...
\begin{equation}
  q_{\mathrm{liq},i-1}=q_{\mathrm{grnd,liq}}+\left(q_{\mathrm{sdew}}-q_{\mathrm{seva}}\right)
\end{equation}
其中$q_{\mathrm{grnd,liq}}$为到达雪盖的液态降水速率，$q_{\mathrm{seva}}$和$q_{\mathrm{sdew}}$分别表示水的蒸发和凝结速率。基于此，顶层液态水含量更新为
\begin{equation}w_{\mathrm{liq},snl+1}^{n+1}=w_{\mathrm{liq},snl+1}^n+\left(q_{\mathrm{grnd,liq}}+q_{\mathrm{sdew}}-q_{\mathrm{seva}}\right)\Delta t
\end{equation}
...
雪层的合并包括以下两种情况：1.考虑固态水的含量。当任何一层几近融化（即$w_{\mathrm{ice},i} \leqslant 0.1$ \unit{kg.m^{-2}}）时，移除该雪层，且将其液态水和固态水含量分配到与之相邻的下层中：
\begin{equation}
