...
该工具基于非结构化一致性三角-六边形构建算法 \citep{fatichi2020soil,walko2008ocean,walko_direct_2011}，以准均匀的全局三角形（Delaunay）的网格化方法为理论依据。其中非结构网格构造流程如图~\ref{fig:非结构化网格生成流程图} 所示，具体介绍如下：\todo{图2.7请补充说明图中iw，im含义}\todo{图2.8部分变量在题注中也需补充说明}
{
  \begin{figure}[htbp]
    \centering
...

{
  \begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{Figures/模式构架/CoLM次网格结构示意图_v4.png}
...

{
  \begin{figure}[htbp]
    ...
    \caption[CoLM积雪分层方案]{CoLM积雪分层方案。当$z_{\mathrm{sno}}<0.01 \unit{mm}$时不形成积雪层，图中厚度单位为\unit{mm}}
    \label{fig:积雪分层}
  \end{figure}
}
...

{
  \begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{Figures/模式构架/CoLM计算框图_v6.png}
    \caption{CoLM计算框图}
    \label{fig:CoLM计算框图}
  \end{figure}
}
