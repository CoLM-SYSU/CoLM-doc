...
\begin{landscape}
  ...
  \begin{equation}\label{eq:建筑能耗平衡方程矩阵}
    ...
    \end{split}
  \end{equation}
  注：上式中$\xi = 0.5 h_{\mathrm{cv,roof}} + 0.5 h_{\mathrm{cv,wsun}}f_{\mathrm{wsun}} + 0.5 h_{\mathrm{cv,wsha}} f_{\mathrm{wsha}}$,$f_{\mathrm{wsun}}$和$f_{\mathrm{wsha}} $为单位建筑面积阳/阴面墙比例，乘积因子$4{\rm HL}$均省略。
\end{landscape}
...
屋顶、阴面墙、阳面墙的温度，$h_{\mathrm{tk,roof}}$、$h_{\mathrm{tk,wsun}}$、$h_{\mathrm{tk,wsha}}$为其与各自室内表面空气温度热交换系数，通过墙壁导热率和厚度计算得到。
