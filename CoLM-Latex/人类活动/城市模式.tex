...
建筑物随机分布，包括建筑物位置及朝向。城市几何参数包括建筑物覆盖度$f_{\rm b}$(即屋顶覆盖面积占比$f_{\mathrm{roof}}$)，地面(透水、不透水面)覆盖度$f_{\rm g}$ ($f_{\mathrm{gimp}}$，$f_{\mathrm{gper}}$)，建筑物高度${\mathrm {H}}$，建筑物高度与建筑物平均边长比$\rm H/L$ (建筑物底面考虑为正方形，边长为记为$\rm L$)，植被冠层中心高度$h_{\rm v}$，城市树木叶面积指数 LAI 及茎面积指数 SAI，植被(树)覆盖面积比例记为$f_{\rm v}$。
...
\end{equation}
\begin{equation}
F_{\mathrm{wg}} \cdot 4 {\rm HL} f_{\mathrm{b}}=F_{\mathrm{s w}} f_{\mathrm{g}}
\end{equation}
...
对于直射辐射太阳辐射，直射光到达阳面墙的辐射通量$E_{\mathrm{wsun}}=S_{\rm w}$，即墙面在地面的阴影覆盖比例。
...
\mathbf{A x}=\mathbf{B}
\end{equation}
$\mathbf{A}$为辐射传输矩阵，$\mathbf{B}$为向量($E_{\mathrm{wsun}}\alpha_{\rm w}$,$E_{\mathrm{wsha}}\alpha_{\rm w}$,
$E_{\mathrm{gimp}}\alpha_{\mathrm{gimp}}$,$E_{\mathrm{gper}}\alpha_{\mathrm{gper}}$)，
...
对于城市水体(湖泊)考虑同屋顶，最终城市区域地表反照率为面积加权，计算方式类似于公式~\eqref{alpha_u2}。
...
\end{equation}
假设该阴影部分与植被覆盖$f_{\rm v}$随机重叠，则植被被阴影遮挡的面积比例为$f_{\rm v}S_{\mathrm {w}} ^\prime$，未覆盖的植被覆盖面积占比为$f_{\rm v}^\prime=f_{\rm v}-f_{\rm v}S_{\rm w}^\prime$。
该未被覆盖植被在整个城市覆盖区域的阴影覆盖$S_{\rm v}$可根据公式(\ref{S_area})计算得到。将其覆盖比例转化为地面所占比例为：
\begin{equation}\label{S_v2}
S_{\mathrm{v}}=S_{\mathrm{v}} / f_{\mathrm{g}}
\end{equation}

假设墙面在地面的阴影占比与植被的阴影随机重叠，则重叠部分占比$S_{\mathrm{vw}}$计算为：
\begin{equation}
...
对于直射辐射太阳辐射，直射光到达阳面墙的辐射通量$E_{\mathrm{wsun}}=S_{\rm w}$，到达阴面墙的辐射通量$E_{\mathrm{wsha}}=S_{\mathrm{vw}}T_{\mathrm {d}}$，
...
第一次反射的长波辐射通量依次分别为：
\begin{equation}
\begin{array}{c}I_{\mathrm{wsun}}\left(1-\varepsilon_{\mathrm{w}}\right) \\ I_{\mathrm{wsha}}\left(1-\varepsilon_{\mathrm{w}}\right) \\ I_{\mathrm{gimp}}\left(1-\varepsilon_{\mathrm{gimp}}\right) \\ I_{\mathrm{gper}}\left(1-\varepsilon_{\mathrm{gper}}\right)\end{array}
\end{equation}
上式中$\varepsilon$表示发射率，$\varepsilon_{\rm w}$，$\varepsilon_{\mathrm{gimp}}$，$\varepsilon_{\mathrm{gper}}$为已知变量。各组分表面向外发射的长波辐射量为：
\begin{equation}
\begin{array}{c}4 f_{\mathrm{wsun}} \mathrm{HL} \cdot \frac{f_{\mathrm{b}}}{f_{\mathrm{g}}} \cdot \sigma \varepsilon_{\mathrm{w}} T_{\mathrm{wsun}}^{4} \\
...
\sigma \varepsilon_{\mathrm{gper}} T_{\mathrm{gper}}^{4} f_{\mathrm{gper}}\end{array}
\end{equation}
...
\begin{equation}
L_{\mathrm{v}}=\left(L_{\mathrm{wsun}}^{\uparrow} F_{\mathrm{wv}}+L_{\mathrm{wsha}}^{\uparrow} F_{\mathrm{wv}}+L_{\mathrm{gimp}}^{\uparrow} F_{\mathrm{gv}}+L_{\mathrm{gper}}^{\uparrow} F_{\mathrm{g v}}+L_{\mathrm{W}} F_{\mathrm{sv}}\right) \varepsilon_{\mathrm{v}}-4 f_{\mathrm{v}} / f_{\mathrm{g}} \sigma \varepsilon_{\mathrm{v}} T_{\mathrm{v}}^{4}
\end{equation}
植被单位面积覆盖长波净辐射修改为：
\begin{equation}
...
城市湍流交换过程与三维植被湍流交换过程类似，采用多层阻抗交换网络计算，包括屋顶、墙面(阴、阳面)、地面和植被的感热、潜热交换量，同样是基于相似性理论，建立每层(等效交换高度)通量守恒方程，联立求解。不同之处在于其粗糙度、迎风面积指数、零平面位移高度、风速/湍流交换系数衰减系数、建筑表面边界层阻抗计算。对于多层阻抗交换网络，屋顶所在高度记为第三层，墙面等效交换高度记为第二层，植被等效交换高度记为第三层。当植被高度与建筑高度相当时，也可以将植被合并于第二层。等效高度计算为建筑物（植被）各自零平面位移加粗糙度的高度。
...
\end{equation}
其中$u_{\mathrm{eff}}$表示屋顶高度/墙面交换高度的等效风速值，$\rho_{\rm a}$表示空气密度，$C_{\rm p}$表示干空气定压比热容。
...
\begin{equation}\label{2lays_noveg_H_layer3}
    H_{3} = H_{2} + H_{\mathrm{wsun}} + H_{\mathrm{wsha}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{2} - T_{3} \right)}{r_{\mathrm{d3}}}
\end{equation}
%
\begin{equation}\label{2lays_noveg_H}
    H = H_{3} + H_{\mathrm{roof}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{3} - T_{\rm a} \right)}{r_{\mathrm{ah}}}
\end{equation}
...
\begin{equation}\label{urban_noveg_Hg}
    H_{\mathrm{g}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{g}} - T_{2} \right) \cdot f_{\mathrm{g}}}{r_{\mathrm{d2}}}
\end{equation}
%
\begin{equation}
    H_{\mathrm{wsun}} = \frac{\rho  _{\rm a} C_{\rm p} \left( T_{\mathrm{wsun}} - T_{2} \right) \cdot f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}}
\end{equation}
%
\begin{equation}
    H_{\mathrm{wsha}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsha}} - T_{2} \right) \cdot f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}}
\end{equation}
%
\begin{equation}\label{urban_noveg_Hroof}
    H_{\mathrm{roof}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{roof}} - T_{3} \right) \cdot f_{\mathrm{roof}}}{r_{\mathrm{roof}}}
\end{equation}
$T_2$和$T_3$分别表示第二层和第三层空气温度，$T_{\mathrm{g}}$为透水面与不透水面温度加权得到的平均温度，$r$为空气动力学阻抗，\allowbreak $f_{\mathrm{roof}}$,\allowbreak  $f_{\mathrm{wsun}}$,\allowbreak  $f_{\mathrm{wsha}}$分表代表屋顶、阳面墙以及阴面墙覆盖比例（$f_{\mathrm{wsun}},f_{\mathrm{wsha}}$应包含乘积因子$4{\rm HL}f_{\rm b}$，为简化计算式表达形式，下面均省略）。$f_{\mathrm{g}}$则为除去建筑覆盖外的路面占比(包含透水面$f_{\mathrm{gper}}$以及不透水面$f_{\mathrm{gimp}}$)，将以上表达式代回公式~\eqref{2lays_noveg_H_layer2}--\eqref{2lays_noveg_H}，可得：
\begin{equation}
    \begin{split}
        & \frac{\rho _{\rm a} C_{\rm p} \left( T_{2} - T_{3} \right)}{r_{\mathrm{d3}}} = \\
        & \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{g}} - T_{2} \right) \cdot f_{\mathrm{g}}}{r_{\mathrm{d2}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsun}} - T_{2} \right) \cdot f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsha}} - T_{2} \right) \cdot f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}}
    \end{split}
\end{equation}
%
\begin{equation}
    \begin{split}
        \frac{\rho _{\rm a} C_{\rm p} \left( T_{3} - T_{\rm a} \right)}{r_{\mathrm{ah}}} =
        \frac{\rho _{\rm a} C_{\rm p} \left( T_{2} - T_{3} \right)}{r_{\mathrm{d3}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{roof}} - T_{3} \right)\cdot f_{\mathrm{roof}}}{r_{\mathrm{roof}}}
    \end{split}
\end{equation}
...
\begin{equation}
\frac{\partial H_{\mathrm{g}}}{\partial T_{\mathrm{g}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{d2}}} \left(1-\frac{f_{\rm g}}{c_{\mathrm{T}} r_{\mathrm{d2}} \left(1-\frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{wsun}}}{\partial T_{\mathrm{wsun}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{wsun}}} \left(1-\frac{f_{\mathrm{wsun}}}{c_{\mathrm{T}} r_{\mathrm{wsun}} \left(1-\frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d2}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{wsha}}}{\partial T_{\mathrm{wsha}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{wsha}}} \left(1-\frac{f_{\mathrm{wsha}}}{c_{\mathrm{T}} r_{\mathrm{wsha}} \left(1-\frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d2}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{roof}}}{\partial T_{\mathrm{roof}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{roof}}} \left(1-\frac{f_{\mathrm{roof}} b_{\mathrm{T}}^2}{c_{\mathrm{T}} r_{\mathrm{roof}} \left(1-\frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d2}}}\right)}-\frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}\left(\frac{1}{r_{\mathrm{ah}}}+\frac{1}{r_{\mathrm{d2}}}+\frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}}\right)}\right)
\end{equation}
...
\begin{equation}
    H_{\mathrm{v}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{v}} - T_{2} \right) \cdot f_{\rm v}}{r_{\mathrm{v}}}
\end{equation}
此时，通量平衡方程变为：
...
\begin{equation}\label{2lays_veg_H_layer3}
    H_{3} = H_{2} + H_{\mathrm{wsun}} + H_{\mathrm{wsha}} + H_{\mathrm{v}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{2} - T_{3} \right)}{r_{\mathrm{d3}}}
\end{equation}
%
...
\begin{equation}
    \begin{split}
        ...
        & \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{g}} - T_{2} \right) \cdot f_{\mathrm{g}}}{r_{\mathrm{d2}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsun}} - T_{2} \right) \cdot f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsha}} - T_{2} \right) \cdot f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}} +
        \\
        & \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{v}} - T_{2} \right) \cdot f_{\rm v}}{r_{\mathrm{v}}}
    \end{split}
\end{equation}
%
\begin{equation}
...
%
\begin{equation}
\frac{\partial H_{\mathrm{wsun}}}{\partial T_{\mathrm{wsun}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{wsun}}} \left(1-\frac{f_{\mathrm{wsun}}}{c_{\mathrm{T}} r_{\mathrm{wsun}} \left(1-\frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{wsha}}}{\partial T_{\mathrm{wsha}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{wsha}}} \left(1-\frac{f_{\mathrm{wsha}}}{c_{\mathrm{T}} r_{\mathrm{wsha}} \left(1-\frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{v}}}{\partial T_{\mathrm{v}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{v}}} \left(1-\frac{f_{\mathrm{v}}}{c_{\mathrm{H}} r_{\mathrm{v}} \left(1-\frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{roof}}}{\partial T_{\mathrm{roof}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{roof}}} \left(1-\frac{f_{\mathrm{roof}} b_{\mathrm{T}}^2}{c_{\mathrm{T}} r_{\mathrm{roof}} \left(1-\frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}-\frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}\left(\frac{1}{r_{\mathrm{ah}}}+\frac{1}{r_{\mathrm{d3}}}+\frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}}\right)}\right)
\end{equation}
...
\end{equation}
$r_{\rm v}$为植被与空气之间水汽交换总阻抗，是叶片边界层阻抗以及气孔阻抗之和($r_{\mathrm{tree}}$+$r_{\mathrm{s}}$)，此时，通量平衡方程可表达为：
...
%
% \begin{equation}
% \frac{\partial  E_{\mathrm{v}}}{\partial T_{\mathrm{v}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{v}}} \frac{{\mathrm {d}} q_{\mathrm{v}}}{{\mathrm {d}} T_{\mathrm{v}}} \left(1-\frac{f_{\mathrm{v}}}{c_{\mathrm{q}} r_{\mathrm{v}}\left(1-\frac{b_{\mathrm{q}}}{c_{\mathrm{q}} r_{\mathrm{v}}}\right)}\right)
% \end{equation}
%
\begin{equation}
\begin{split}
...
\begin{equation}\label{urban_Hg_3_veg}
    H_{\mathrm{g}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{g}} - T_{1} \right) f_{\mathrm{g}}}{r_{\mathrm{d1}}}
\end{equation}
%
\begin{equation}
    H_{\mathrm{v}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{v}} - T_{1} \right) f_{\rm v}}{r_{\mathrm{v}}}
\end{equation}
%
\begin{equation}
    H_{\mathrm{wsun}} = \frac{\rho  _{\rm a} C_{\rm p} \left( T_{\mathrm{wsun}} - T_{2} \right) f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}}
\end{equation}
%
\begin{equation}
    H_{\mathrm{wsha}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsha}} - T_{2} \right) f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}}
\end{equation}
%
\begin{equation}\label{urban_Hroof_3_veg}
    H_{\mathrm{roof}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{roof}} - T_{\mathrm{3}} \right) f_{\mathrm{roof}}}{r_{\mathrm{roof}}}
\end{equation}
其各项含义两层方案相同(章节~\ref{两层方案_植被})，此处不再赘述，将以上表达式代回公式~\eqref{3lay_H1_veg}-~\eqref{3lay_H3_veg}可得：
%
\begin{equation}
  \frac{\rho _{\rm a} C_{\rm p} \left( T_{1} - T_{2} \right)}{r_{\mathrm{d3}}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{g}} - T_{1} \right) f_{\mathrm{g}}}{r_{\mathrm{d1}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{v}} - T_{1} \right) f_{\rm v}}{r_{\mathrm{v}}}
\end{equation}
%
\begin{equation}
    \begin{split}
        & \frac{\rho _{\rm a} C_{\rm p} \left( T_{2} - T_{3} \right)}{r_{\mathrm{d3}}} = \\
        & \frac{\rho _{\rm a} C_{\rm p} \left( T_{1} - T_{2} \right) }{r_{\mathrm{d2}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsun}} - T_{2} \right) \cdot f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsha}} - T_{2} \right) \cdot f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}}
    \end{split}
\end{equation}
%
\begin{equation}
...
\begin{equation}
\frac{\partial H_{\mathrm{g}}}{\partial T_{\mathrm{g}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{d1}}} \left(1-\frac{f_{\rm g}}{r_{\mathrm{d1}} \left(\frac{1}{r_{\mathrm{d2}}}+\frac{f_{\rm g}}{r_{\mathrm{d1}}}+\frac{f_{\rm v}}{r_{\rm v}}\right)} - \frac{f_{\rm g} a_{\mathrm{T}}^2}{r_{\mathrm{d1}} c_{\mathrm{T}} \left(1-\frac{a_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d2}}}-\frac{b_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{wsun}}}{\partial T_{\mathrm{wsun}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{wsun}}} \left(1-\frac{f_{\mathrm{wsun}}}{c_{\mathrm{T}} r_{\mathrm{wsun}} \left(1-\frac{a_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d2}}}-\frac{b_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{wsha}}}{\partial T_{\mathrm{wsha}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{wsha}}} \left(1-\frac{f_{\mathrm{wsha}}}{c_{\mathrm{T}} r_{\mathrm{wsha}} \left(1-\frac{a_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d2}}}-\frac{b_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{v}}}{\partial T_{\mathrm{v}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{v}}} \left(1-\frac{f_{\rm v}}{r_{\mathrm{v}} \left(\frac{1}{r_{\mathrm{d2}}}+\frac{f_{\rm g}}{r_{\mathrm{d1}}}+\frac{f_{\rm v}}{r_{\rm v}}\right)} - \frac{f_{\rm v} a_{\mathrm{T}}^2}{r_{\mathrm{v}} c_{\mathrm{T}} \left(1-\frac{a_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d2}}}-\frac{b_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{roof}}}{\partial T_{\mathrm{roof}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{roof}}} \left(1-\frac{f_{\mathrm{roof}} b_{\mathrm{T}}^2}{c_{\mathrm{T}} r_{\mathrm{roof}} \left(1-\frac{a_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d2}}}-\frac{b_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}-\frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}\left(\frac{1}{r_{\mathrm{ah}}}+\frac{1}{r_{\mathrm{d3}}}+\frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}}\right)}\right)
\end{equation}
%-------------
%潜热通量
%-------------
...
对于透水面的处理同土壤水过程，计算产流和土壤水传输。对于城市水体，采用湖泊方案进行模拟。对于屋顶和不透水面，考虑积雪和积水过程。积雪过程与自然土壤积雪过程一致。积水过程考虑表层为不透水面，液态水的最大承载量不超过预先设定的值($\text{max ponding}=1$ \unit{kg.m^{-2}})，超过的部分当成产流处理。积水区覆盖占比采用类似叶片$f_{\mathrm{wet}}$计算方案(公式~\eqref{eq:fwet})。
...
屋顶、阴面墙、阳面墙的温度，为$h_{\mathrm{tk,roof}}$、$h_{\mathrm{wsun,tk}}$、$h_{\mathrm{wsha,tk}}$为其与各自室内表面空气温度热交换系数，通过墙壁导热率和厚度计算得到。
...
\end{equation}
其中$V_{\mathrm {c}}$、$V_{\mathrm {M}}$、$V_{\mathrm{FR}}$分别为每千人拥有的汽车、摩托车、货车(巴士)的数量，$H_{\mathrm{traf}}$为工作日/休息日某一时间段交通流量，以上数据采用 \citet{allen2011}；$E_{\mathrm {c}}$、$E_{\mathrm {M}}$、$E_{\mathrm{FR}}$则为三种机动车的排放系数，目前使用 \citet{sailor2004top} 的系数设置(\qty{3975}{J.m^{-1}})；
...
\begin{equation}\label{2lays_H_layer3}
    H_{3} = H_{2} + H_{\mathrm{wsun}} + H_{\mathrm{wsha}} + H_{\mathrm{v}} + H_{\mathrm{whac}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{2} - T_{3} \right)}{r_{\mathrm{d3}}}
\end{equation}
%
\begin{equation}\label{2lays_H}
    H = H_{3} + H_{\mathrm{roof}} + H_{\mathrm{rhac}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{3} - T_{\rm a} \right)}{r_{\mathrm{ah}}}
\end{equation}
...
\begin{equation}\label{urban_Hg}
    H_{\mathrm{g}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{g}} - T_{2} \right) \cdot f_{\mathrm{g}}}{r_{\mathrm{d2}}} + H_{\mathrm{vehc}} + H_{\mathrm{meta}}
\end{equation}
%
\begin{equation}
    H_{\mathrm{wsun}} = \frac{\rho  _{\rm a} C_{\rm p} \left( T_{\mathrm{wsun}} - T_{\mathrm{2}} \right) \cdot f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}}
\end{equation}
%
\begin{equation}
    H_{\mathrm{wsha}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsha}} - T_{\mathrm{2}} \right) \cdot f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}}
\end{equation}
%
\begin{equation}
    H_{\mathrm{v}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{v}} - T_{2} \right) \cdot f_{\rm v}}{r_{\mathrm{v}}}
...
\begin{equation}
    \begin{split}
        ...
        & \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{g}} - T_{2} \right) \cdot f_{\mathrm{g}}}{r_{\mathrm{d2}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsun}} - T_{2} \right) \cdot f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsha}} - T_{2} \right) \cdot f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}} +
        \\
        & \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{v}} - T_{2} \right) \cdot f_{\rm v}}{r_{\mathrm{v}}} + H_{\mathrm{AHE3}}
...
\begin{equation}
    \begin{split}
        \frac{\rho _{\rm a} C_{\rm p} \left( T_{3} - T_{\rm a} \right)}{r_{\mathrm{ah}}} =
        \frac{\rho _{\rm a} C_{\rm p} \left( T_{2} - T_{3} \right)}{r_{\mathrm{d3}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{roof}} - T_{3} \right)\cdot f_{\mathrm{roof}}}{r_{\mathrm{roof}}} + H_{\mathrm{AHE3}}
    \end{split}
\end{equation}
其中：
\begin{equation}
...
\begin{equation}
    \begin{split}
         T_{2} =
         \frac{\frac{T_{\mathrm{g}} \cdot f_{\mathrm{g}}}{r_{\mathrm{d2}}} + \frac{H_{\mathrm{AHE2}}}{\rho _{\rm a} C_{\rm p}} + \frac{T_{\mathrm{wsun}} \cdot f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{T_{\mathrm{wsha}} \cdot f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}} + \frac{T_{\mathrm{v}} \cdot f_{\rm v}}{r_{\mathrm{v}}} + \frac{T_{3}}{r_{\mathrm{d3}}}}{\frac{1}{r_{\mathrm{d3}}} + \frac{f_{\mathrm{g}}}{r_{\mathrm{d2}}} + \frac{f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}} + \frac{f_{\rm v}}{r_{\mathrm{v}}}}
    \end{split}
\end{equation}
%
\begin{equation}\label{result_T_layer3}
    T_{3} = \frac{\frac{T_{2}}{r_{\mathrm{d3}}} + \frac{T_{\mathrm{roof}} \cdot f_{\mathrm{roof}}}{r_{\mathrm{roof}}} + \frac{H_{\mathrm{AHE3}}}{\rho _{\rm a} C_{\rm p}} + \frac{T_{\rm a}}{r_{\mathrm{ah}}}}{\frac{1}{r_{\mathrm{ah}}} + \frac{1}{r_{\mathrm{d3}}} + \frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}}}
\end{equation}
求解该二元一次方程可得：
\begin{equation}
    \begin{split}
    % \hspace{-1cm}
         T_{2} =
         \frac{\frac{T_{\mathrm{g}} \cdot f_{\mathrm{g}}}{r_{\mathrm{d2}}} + \frac{H_{\mathrm{AHE2}}}{\rho _{\rm a} C_{\rm p}} + \frac{T_{\mathrm{wsun}} \cdot f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{T_{\mathrm{wsha}} \cdot f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}} + \frac{T_{\mathrm{v}} \cdot f_{\mathrm{v}}}{r_{\mathrm{v}}} + a_{\mathrm{T}}}{c_{\mathrm{T}} \cdot \left( 1 - \frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d3}}} \right)}
    \end{split}
\end{equation}
%
其中：
\begin{equation}
    a_{\mathrm{T}} = \left(\frac{T_{\mathrm{roof}} \cdot f_{\mathrm{roof}}}{r_{\mathrm{roof}}} + \frac{H_{\mathrm{AHE3}}}{\rho _{\rm a} C_{\rm p}} + \frac{T_{\rm a}}{r_{\mathrm{ah}}}\right) \cdot b_{\mathrm{T}}
\end{equation}
%
\begin{equation}
    b_{\mathrm{T}} = \frac{1}{r_{\mathrm{d3}} \cdot \left(\frac{1}{r_{\mathrm{ah}}} + \frac{1}{r_{\mathrm{d3}}} + \frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}} \right)}
...
%
\begin{equation}
\frac{\partial H_{\mathrm{v}}}{\partial T_{\mathrm{v}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{v}}} \left(1-\frac{f_{\mathrm{v}}}{c_{\mathrm{T}} r_{\mathrm{v}} \left(1-\frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}\right)
\end{equation}
%
\begin{equation}
\frac{\partial H_{\mathrm{roof}}}{\partial T_{\mathrm{roof}}} = \frac{\rho _{\rm a} C_{\rm p}}{r_{\mathrm{roof}}} \left(1-\frac{f_{\mathrm{roof}} b_{\mathrm{T}}^2}{c_{\mathrm{T}} r_{\mathrm{roof}} \left(1-\frac{b_{\mathrm {T}} }{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}-\frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}\left(\frac{1}{r_{\mathrm{ah}}}+\frac{1}{r_{\mathrm{d3}}}+\frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}}\right)}\right)
...
\begin{equation}\label{urban_Hg_3}
    H_{\mathrm{g}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{g}} - T_{1} \right) f_{\mathrm{g}}}{r_{\mathrm{d1}}} + H_{\mathrm{vehc}} + H_{\mathrm{meta}}
\end{equation}
%
\begin{equation}
    H_{\mathrm{v}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{v}} - T_{1} \right) f_{\rm v}}{r_{\mathrm{v}}}
...
\begin{equation}\label{urban_Hroof_3}
    H_{\mathrm{roof}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{roof}} - T_{3} \right) f_{\mathrm{roof}}}{r_{\mathrm{roof}}}
\end{equation}
其各项含义两层方案相同(章节~\ref{人为热两层方案})，此处不再赘述，将以上表达式代回公式~\eqref{3lay_H1}-~\eqref{3lay_H3}可得：
%
\begin{equation}
    \frac{\rho _{\rm a} C_{\rm p} \left( T_{1} - T_{2} \right)}{r_{\mathrm{d3}}} = \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{g}} - T_{1} \right) f_{\mathrm{g}}}{r_{\mathrm{d1}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{v}} - T_{1} \right) f_{\rm v}}{r_{\mathrm{v}}} + H_{\mathrm{AHE1}}
\end{equation}
%
\begin{equation}
    \begin{split}
        & \frac{\rho _{\rm a} C_{\rm p} \left( T_{2} - T_{3} \right)}{r_{\mathrm{d3}}} = \\
        & \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{1}} - T_{2} \right) }{r_{\mathrm{d2}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsun}} - T_{2} \right) \cdot f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{\rho _{\rm a} C_{\rm p} \left( T_{\mathrm{wsha}} - T_{2} \right) \cdot f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}} + H_{\mathrm{AHE2}}
    \end{split}
\end{equation}
%
\begin{equation}
...
\begin{equation}
\label{3lay_T1}
    T_{1} =\frac{\frac{T_{\mathrm{g}} f_{\mathrm{g}}}{r_{\mathrm{d1}}} + \frac{T_{\mathrm{v}} f_{\rm v}}{r_{\mathrm{v}}} + \frac{T_{\mathrm{2}}}{r_{\mathrm{d2}}} + \frac{H_{\mathrm{AHE1}}}{\rho _{\rm a} C_{\rm p}}}{\frac{1}{r_{\mathrm{d2}}} + \frac{f_{\mathrm{g}}}{r_{\mathrm{d1}}} + \frac{f_{\rm v}}{r_{\mathrm{v}}}}
\end{equation}
%
\begin{equation}
    T_{2} = \frac{\frac{T_{\mathrm{wsun}} f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{T_{\mathrm{wsha}} f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}} +  \frac{T_{1}}{r_{\mathrm{d2}}} + \frac{T_{\mathrm{3}}}{r_{\mathrm{d3}}} + \frac{H_{\mathrm{AHE2}}}{\rho _{\rm a} C_{\rm p}}}{\frac{1}{r_{\mathrm{d3}}} + \frac{1}{r_{\mathrm{d2}}} + \frac{f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}}}
\end{equation}
%
\begin{equation}
\label{3lay_T3}
    T_{3} = \frac{\frac{T_{\mathrm{roof}} f_{\mathrm{roof}}}{r_{\mathrm{roof}}} + \frac{T_{\mathrm{2}}}{r_{\mathrm{d3}}} + \frac{H_{\mathrm{AHE3}}}{\rho _{\rm a} C_{\rm p}} + \frac{T_{\rm a}}{r_{\mathrm{ah}}}}{\frac{1}{r_{\mathrm{ah}}} + \frac{1}{r_{\mathrm{d3}}} + \frac{f_{\mathrm{roof}}}{r_{\mathrm{roof}}}}
\end{equation}
求解该方程可得：
\begin{equation}
\begin{split}
    T_{2} = \frac{\frac{T_{\mathrm{wsun}} f_{\mathrm{wsun}}}{r_{\mathrm{wsun}}} + \frac{T_{\mathrm{wsha}} f_{\mathrm{wsha}}}{r_{\mathrm{wsha}}} + \left(\frac{T_{\mathrm{g}} f_{\mathrm{g}}}{r_{\mathrm{d1}}} + \frac{T_{\mathrm{v}} f_{\mathrm{v}}}{rv} + \frac{H_{\mathrm{AHE1}}}{\rho _{\rm a} C_{\rm p}}\right)a_{\mathrm{T}} + \left(\frac{T_{\mathrm{roof}} f_{\mathrm{roof}}}{r_{\mathrm{roof}}} + \frac{T}{r_{\mathrm{ah}}} + \frac{H_{\mathrm{AHE3}}}{\rho _{\rm a} C_{\rm p}}\right)b_{\mathrm{T}} + \frac{H_{\mathrm{AHE2}}}{\rho _{\rm a} C_{\rm p}}}{c_{\mathrm{T}} \left(1-\frac{a_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d2}}}-\frac{b_{\mathrm{T}}}{c_{\mathrm{T}} r_{\mathrm{d3}}}\right)}
\end{split}
\end{equation}
其中：
\begin{equation}
