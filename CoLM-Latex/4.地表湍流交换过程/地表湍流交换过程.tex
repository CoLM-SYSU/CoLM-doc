...
不稳定边界层的大涡对地表湍流交换具有重要影响，随着研究和认识的不断深入，近年来在考虑大涡影响的基本理论方面有了新进展。\citet{liu2019further,liu2022surface}
通过引入边界层高度$z_i$，提出了包含大涡影响的动量通量通用函数形式：
\begin{equation}
     \begin{array}{c}\phi_{m}(\zeta)=C_{m} (-\zeta)^{-1/2} \\
          C_{m}=0.0047(-\frac{z_{i}}{L})+0.1854
     \end{array}
\end{equation}
该方案适用于不稳定条件($\zeta \leq -0.13$)。在弱不稳定条件下，通用函数则继续采用方程(\ref{phim_zeng})所列形式。
