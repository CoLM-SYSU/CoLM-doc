...
本部分的对应代码为\texttt{groundfluxes.F90}、\texttt{FRICTION\-\_VELOCITY.F90}和\texttt{LEAF\-\_temper\-ature.F90}。
...
\begin{equation}
\zeta=
\left\{\begin{array}{ll}\frac{R_{i b} \ln \left(\frac{z_{atm, m}-d}{z_{0 m}}\right)}{1-5 \min \left(R_{i b}, 0.19\right)} & 10^{-6} \leq \zeta \leq 2, R_{i b} \geq 0  \, \text{（即中性或稳定条件下）}
    \\ R_{i b} \ln \left(\frac{z_{atm, m}-d}{z_{0 m}}\right) & -100 \leq \zeta \leq-10^{-6}, R_{i b}<0 \, \text{（即中性或稳定条件下）}\end{array}\right.
\end{equation}
