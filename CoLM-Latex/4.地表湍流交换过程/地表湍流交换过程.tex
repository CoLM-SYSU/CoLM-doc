...
为方便与地面观测资料进行比较，可定义2m温度和湿度、以及10m风速，它们实际上为$z_{0x}+d$以上2m高度的温度、比湿，和10m高度的风速(模式暂未计算)，计算公式(即对原始微分方程从$z_{0x}+d$到$z_{0x}+d+2$ (或$z_{0x}+d+10$) 进行积分)如下：
...
\begin{equation}
\left\{\begin{array}{lr}\phi_{m}(\zeta)=B_{m}(-\zeta)^{-1/2}, & \zeta<\zeta_{m2} \text {时} \\
      \phi_{m}(\zeta)=(1-16 \zeta)^{-1/4}, & \zeta_{m2} \leq \zeta<0 \text {时} \\
       \end{array}\right.
\end{equation}
其中$\zeta_{m2}=\min(\zeta_{m},-0.13)$，稳定条件下的$\phi_{m}$则直接采用方程(\ref{phim_zeng})中形式。基于风速廓线方程(\ref{Va})，注意当
...
\psi_{m}\left(\zeta_{atm}\right) =& \int_{{z_{0x}}/{L}}^{\zeta_{atm}} \frac{1-\phi_{m}(s)}{s} d s  \nonumber \\
 ...
 =& \psi_{mu}(\zeta_{m2}) + \int_{\zeta_{m2}}^{\zeta_{atm}} \frac{1-B_{m}(-s)^{-1/2}}{s} d s  \nonumber \\
 =& \psi_{mu}(\zeta_{m2}) + \ln \frac{\zeta_{atm}}{\zeta_{m2}} + 2B_{m}\left[(-\zeta_{atm})^{-1/2}-(-\zeta_{m2})^{-1/2}\right]
\end{align}
可得到风速廓线在不稳定条件下的具体形式：

\noindent 当$\zeta_{atm}<\zeta_{m2}$时
\begin{equation}\label{Va_U_LZD1}
V_{a}=\frac{u_{*}}{k}\left\{\ln \frac{L\zeta_{m2}}{z_{0 m}}-\psi_{mu}\left(\zeta_{m2}\right)-2B_{m}\left[(-\zeta_{atm})^{-1/2}-(-\zeta_{m2})^{-1/2}\right]+\psi_{m}\left(\frac{z_{0 m}}{L}\right)\right\}
\end{equation}
...
$C_s=C_{s,bare}W+C_{s,dense}\left(1-W\right)$，其中权重$W=\exp {-\left(LAI+SAI\right)}$，
